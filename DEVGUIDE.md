...
## Developing compiler service using the released version of .NET and without using Proto compiler (advanced scenario, fast dev loop)

> **Warning**
> This is an advanced scenario, and is not necessary for most development. It is only recommended for those who are working on the compiler service and know what they're doing.
>
> **Please, keep in mind**, that this scenario is experimental and not officially supported, as for now, the only official supported way of building compiler is to use build scripts.

There are multiple ways of building `FSharp.Compiler.Service.`sln` using the released version of .NET:

* Build `FSharp.Compiler.Service.sln` normally - `dotnet build FSharp.Compiler.Service.sln`.

* In case of a different solution/project, pass `BUILDING_USING_DOTNET` flag to the `dotnet build` or `msbuild`:

```shell
dotnet build src/Compiler/FSharp.Compiler.Service.fsproj /p:BUILDING_USING_DOTNET=true
```

* Create a file, named `Directory.Build.props.user` in repository root, with the `BUILDING_USING_DOTNET` property set to `true`, and then build using `dotnet build` or `msbuild`:
...
## Updating parser and lexer

Parser and lexer outputs are cached in the repository and are regenerated automatically every time fslexyacc input files are changed (fsl and fsy files), however, they will not be regenerated if any dependent compiler libraries are changed.
If you are making changes to the latter, and do not see your changes in the parser/lexer, you will need to manually regenerate the outputs. This can be done by removing `src\.autogenerated\fslexyacc` directory, and running build again.
