...
After checking errors are clean, run tests with cached compilation (skips full recompile of FSharp.Compiler.Service and FSharp.Compiler.ComponentTests):
```bash
...
```
All other projects build normally. Falls back to normal build if server is unavailable.
