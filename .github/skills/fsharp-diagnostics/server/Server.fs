                ...
                try
                    ...
                    | "parseOnly" ->
                        ...
                        if not (File.Exists file) then
                            ...
                            let parsingOpts, _ =
                                checker.GetParsingOptionsFromCommandLineArgs(file :: parsingArgs)
                        ...
                        if not (File.Exists file) then
                            ...
                            | Ok options ->
                                ...
                                | FSharpCheckFileAnswer.Succeeded checkResults ->
                                    ...
                                    while endCol < lineText.Length && isIdChar lineText.[endCol] do
                                        endCol <- endCol + 1

                                    let mutable startCol = col

                                    while startCol > 0 && isIdChar lineText.[startCol - 1] do
                                        startCol <- startCol - 1
                                    ...
                                    if name.Length = 0 then
                                        ...
                                        | Some symbolUse ->
                                            ...
                                            | :? FSharpEntity as ent when ent.IsFSharpUnion ->
                                                for uc in ent.UnionCases do
                                                    targetNames.Add(uc.FullName)
                                            | _ -> ()
                                            ...
                                            let rel (p: string) =
                                                if p.StartsWith(root) then p.Substring(root.Length) else p

                                            let lines =
                                                uses
                                                |> Array.map (fun u ->
                                                    let kind =
                                                        if u.IsFromDefinition then "DEF"
                                                        elif u.IsFromType then "TYPE"
                                                        else "USE"
                                            ...
                                            let header =
                                                $"Symbol: {sym.DisplayName} ({sym.GetType().Name}) — {lines.Length} references"
                            ...
                            | Ok options ->
                                ...
                                | FSharpCheckFileAnswer.Succeeded checkResults ->
                                    ...
                                    let addHint line hint =
                                        if not (annotations.ContainsKey line) then
                                            annotations.[line] <- ResizeArray()
                                        ...
                                        if r.StartLine >= startLine && r.StartLine <= endLine && su.IsFromDefinition then
                                            ...
                                            | :? FSharpMemberOrFunctionOrValue as mfv ->
                                                ...
                                                | None ->
                                                    ...
                                                    try
                                                        addHint r.StartLine $"({mfv.DisplayName}: {mfv.FullType.Format(su.DisplayContext)})"
                                                    with _ ->
                                                        ()
                                            | :? FSharpField as fld ->
                                                try
                                                    addHint r.StartLine $"({fld.DisplayName}: {fld.FieldType.Format(su.DisplayContext)})"
                                                with _ ->
                                                    ()
                                            | _ -> ()
