...
description: "Review dotnet/fsharp performance-improvement PRs for measured value, appropriate benchmarks, behavior preservation, and surgical implementation risk using source-attributed performance-review precedents."
---
...
| 3. Metric selection and measurement validity | PERF-03: use the map below; explain why requested metrics matter. Check identical inputs/configurations, measurement boundaries, warmup, cold/warm caches and GC policy. For capability-dependent fast paths, compare representative supported fast/fallback paths on the same runtime (e.g. hardware intrinsics enabled/disabled); another runtime's results do not establish fallback cost. Structural counts are not measured allocation traffic or time; an allocation win need not improve timing. |
...
| 5. Regressions and resource trade-offs | PERF-05: trace ownership, retained state/leaks, release/eviction and repeated/incremental work. Check implicated memory/GC, CPU, synchronization, unsafe concurrency, threads, thread-pool pressure, starvation/hangs and cancellation. When changing instrumentation, credit repaired ownership separately from remaining collection costs: per-instance tags/filtering, direct counters and active exporters. Request relevant enabled/disabled cost evidence or justify a simpler bounded mechanism; useful diagnostics need not be removed or gated by a particular directive. Allocation volume is not live memory; caches are not inherently leaks, nor weak references lifetime proof. |
...
A source-established failure for previously valid callers is `risk/blocker`, not merely a request to decide compatibility policy. Distinguish an explicitly intended behavior change from its separate collateral effects; intentional optimization is not a blanket exemption.
