...
build: proto restore
	...
	$(DotNetExe) build -c $(Configuration) -f netcoreapp2.0 tests/FSharp.Compiler.UnitTests/FSharp.Compiler.UnitTests.fsproj
...
test: build
	$(DotNetExe) test -f netcoreapp2.0 -c $(Configuration) --no-restore --no-build tests/FSharp.Compiler.UnitTests/FSharp.Compiler.UnitTests.fsproj -l "trx;LogFileName=$(CURDIR)/tests/TestResults/FSharp.Compiler.UnitTests.coreclr.trx"
