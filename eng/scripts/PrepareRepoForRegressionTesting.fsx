...
if File.Exists(propsFilePath) then
    ...
    if isNull otherFlagsWithTimes then
        ...
        // Clean up --nowarn:75 from OtherFlags if present (moved to <NoWarn> element)
        if otherFlagsWithTimes.InnerText.Contains("--nowarn:75") then
            otherFlagsWithTimes.InnerText <- otherFlagsWithTimes.InnerText.Replace(" --nowarn:75", "").Replace("--nowarn:75 ", "")
        
        let parentPG = otherFlagsWithTimes.ParentNode
        let existingNoWarn = parentPG.SelectSingleNode("NoWarn[contains(text(), '0075')]")
