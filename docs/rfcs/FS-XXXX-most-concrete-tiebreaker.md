...
The design suggestion [Prefer more concrete overloads in overload resolution](https://github.com/fsharp/fslang-suggestions/issues/FILL-ME-IN) has been marked "approved in principle".
...
The tiebreaker integrates naturally with SRTP (statically resolved type parameters). When an inline function with SRTP constraints calls overloaded methods, the tiebreaker applies at instantiation time when concrete types are known. SRTP type variables themselves (`^T`) are not compared for concreteness since they represent deferred constraints, but the concrete types at each call site participate fully in the comparison.

# Unresolved Questions

None. The design is complete and implemented.
