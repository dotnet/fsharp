...
*)
let x0 : FSharp.Compiler.Symbols.FSharpSymbol = failwith ""
let x1 : FSharp.Compiler.Symbols.FSharpAbstractParameter = failwith ""
let x1 : FSharp.Compiler.Symbols.FSharpAbstractSignature = failwith ""
let x2 : FSharp.Compiler.Symbols.FSharpAccessibility = failwith ""
let x3 : FSharp.Compiler.Symbols.FSharpAccessibilityRights = failwith ""
let x4 : FSharp.Compiler.Symbols.FSharpActivePatternCase = failwith ""
let x5 : FSharp.Compiler.Symbols.FSharpExpr = failwith ""
let x6 : FSharp.Compiler.Symbols.FSharpMemberOrFunctionOrValue = failwith ""
let x7 : FSharp.Compiler.Symbols.FSharpEntity = failwith ""
let x8 : FSharp.Compiler.Symbols.FSharpField = failwith ""
let x9 : FSharp.Compiler.Symbols.FSharpUnionCase = failwith ""
let x10 : FSharp.Compiler.Symbols.FSharpGenericParameter = failwith ""
let x11 : FSharp.Compiler.Symbols.FSharpParameter = failwith ""
let x12 : FSharp.Compiler.Symbols.FSharpType = failwith ""
let x13 : FSharp.Compiler.Symbols.FSharpOpenDeclaration = failwith ""
let x14 : FSharp.Compiler.Symbols.FSharpDisplayContext = failwith ""
let x15 : FSharp.Compiler.Symbols.FSharpAttribute = failwith ""
let x16 : FSharp.Compiler.CodeAnalysis.FSharpCheckFileResults  = failwith ""
...
// merge CompilerEnvironment, DebuggerEnvironment, FSHarpFileUtilities, FSHarpSourceFile, into FSharp.Compiler.Environment 

// CodeAnalysis.FSharpExternalSymbol --> EditorServices.FindDeclExternalSymbol (?)

// getAssemblyContent --> uppercase

// AssemblyContentProvider --> AssemblyContent

// Hide AddIndexerDot, ReplaceWithSuggestion

// Hide SynModuleOrNamespace etc under FSharp.Compiler.Syntax

// Decision on whether to switch everything to ImmutableArray
...
// Three ways of representing XML doc 
//     SyntaxTree uses a type FSharp.Compiler.Syntax.XmlDoc
//     ToolTip (FSharpToolTipElementData) uses a type FSharp.Compiler.EditorServices.FSharpXmlDoc
//     Symbols uses three members sym.XmlDoc, sym.XmlDocSig, sym.ElaboratedXmlDoc
// Rationalise this to one ot two mechanisms

// Remove "ExtraProjectInfo" and its propagation to events 
// e.g. "obj option" on BeforeBackgroundFileCheck event data

// Note on these:
//    checker.GetBackgroundSemanticClassificationForFile
//    checker.GetBackgroundCheckResultsForFileInProject
// These are needed because we don't keep the SemanticClassification information for background files
// and it may need a separate request.
//
// Double check this with Will.

// NotifyProjectCleaned --> mark as Obsolete

// checker.ParseFileNoCache - why not pass a boolean flag?

// Tokenization entry points, several ways to do the same thing:
//
//   checker.TokenizeFile
//   checker.TokenizeLine
//   FSharp.Compiler.Tokenization.FSharpLexer.Tokenize
//   FSharp.Compiler.Tokenization.FSharpLineTokenizer
//   FSharp.Compiler.Tokenization.FSharpSourceTokenizer
//
// Should we rationalise these?
