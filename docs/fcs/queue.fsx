...
* "Runs on caller thread (runs on caller thread)" - Some requests from FSharp.Editor are serviced concurrently without using the queue at all. Everything without
in Async return type are in this category.

* "Queued-at-high-priority (runs on reactor thread)" - These are requests made via the FCS API (e.g. from FSharp.Editor) and anything with "Async" in the return type is in this category. The originating calls are not typically on the UI thread and are associated with active actions by the user (editing a file etc.). 

* "Queued and interleaved at lower priority (runs on reactor thread)" - This is reserved for a "background" job (CheckProjectInBackground) used for to prepare the project builder state of the current project being worked on. The "background" work is intended to be divided into little chunks so it can always be interrupted in order to service the higher-priority work.

The "Queued-at-lower-priority" operation runs as a low-priority, interleaved operation when the
queue is empty.  This is used to implicitly bring the background check of a project "up-to-date".  
...
Several calls to the FSharpChecker API enqueue an operation in the FSharpChecker compiler queue (the "Queued-at-high-priority" category).
...
should be cautious about operations that request a check of the entire project - these
operations will be "Queued-as-high-priority" and run in preference to other similar operations
and must be both asynchronous and cancelled if the results will no longer be needed.
...
The long term intent of FCS is to eventually remove the queue altogether. However the queue
...
2. It acts as a data-lock on the project builder compilation state, not all of which is thread safe (though we keep reviewing it).
