...
* Refactoring to switch a single-case partial active pattern between returning `option` and returning a struct `voption` (`[<return: Struct>]`), rewriting the `Some`/`None` results and the return type annotation; match sites stay unchanged.
