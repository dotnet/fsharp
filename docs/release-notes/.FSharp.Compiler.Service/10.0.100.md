...
* Fix tail call emission when method contains pinned locals to prevent runtime crashes and memory corruption. Methods with pinned locals now suppress the `.tail.` prefix to ensure memory safety.
