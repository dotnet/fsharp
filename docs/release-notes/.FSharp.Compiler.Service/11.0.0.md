...
* Added support for `System.Runtime.CompilerServices.OverloadResolutionPriorityAttribute` (.NET 9). Higher priority overloads are preferred during overload resolution. Optional informational diagnostic FS3590 reports when priority affects resolution (enabled with `--warnon:3590`). Requires `--langversion:preview`. ([PR TBD](https://github.com/dotnet/fsharp/pull/))
