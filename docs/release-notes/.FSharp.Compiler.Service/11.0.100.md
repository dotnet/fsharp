...
* Expand `<inheritdoc/>` at tooling time — in the FCS Symbols API (`FSharpSymbol.XmlDoc`) and in IDE tooltips, completion, and signature help — resolving documentation from base classes, interfaces, overridden members, and (via the Symbols API) explicit `cref` targets. The compiler emits the tag verbatim into generated XML documentation files, matching C#; `<include>` is not implemented. ([Issue #19175](https://github.com/dotnet/fsharp/issues/19175), [PR #19188](https://github.com/dotnet/fsharp/pull/19188))
