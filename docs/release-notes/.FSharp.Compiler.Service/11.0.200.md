...
* Add experimental F# hot reload with baseline capture, runtime deltas, rude-edit detection, and project sessions. `--test:HotReloadDeltas` enables the feature. `--test:HotReloadClassStateMachines` enables reference-type state machines for resume-point edits. Both flags remain off by default. ([Issue #11636](https://github.com/dotnet/fsharp/issues/11636), [PR #19941](https://github.com/dotnet/fsharp/pull/19941))
* Add an experimental in-process compiler path for hot reload sessions through `FSHARP_HOTRELOAD_INPROCESS_COMPILE`. The nested `FSHARP_HOTRELOAD_INCREMENTAL_EMIT` flag enables an optimized prefix cache. ([PR #20031](https://github.com/dotnet/fsharp/pull/20031))
* Add the experimental `FSharpChecker.CreateHotReloadSession` API with per-project baselines, capability checks, and commit or discard operations. Distinct `--out:` paths retain independent baselines. ([Issue #11636](https://github.com/dotnet/fsharp/issues/11636), [PR #20030](https://github.com/dotnet/fsharp/pull/20030))
