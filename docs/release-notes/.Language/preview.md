...
* Removed the always-on `StructActivePattern` language feature flag; it shipped enabled for every selectable `--langversion` since F# 6.0, so `--disableLanguageFeature:StructActivePattern` is no longer a recognised feature name. ([Issue #20161](https://github.com/dotnet/fsharp/issues/20161), [PR #20208](https://github.com/dotnet/fsharp/pull/20208))
