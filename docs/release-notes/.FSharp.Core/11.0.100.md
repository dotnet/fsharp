...
* Fix `Array2D` AOT compatibility: `create` and `init` no longer emit `IL3050`, and the `*Based` operations now warn at the call site before the runtime failure they already had. ([Language Suggestion #1454](https://github.com/fsharp/fslang-suggestions/issues/1454), [PR #20338](https://github.com/dotnet/fsharp/pull/20338))
