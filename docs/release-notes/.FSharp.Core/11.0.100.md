...
* Fix `Array2D` AOT compatibility on `net10.0`: zero-based operations no longer emit `IL3050`, and the `*Based` ones now warn at the call site instead of failing at runtime. ([Language Suggestion #1454](https://github.com/fsharp/fslang-suggestions/issues/1454), [PR #20338](https://github.com/dotnet/fsharp/pull/20338))
