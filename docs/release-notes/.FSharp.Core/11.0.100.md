...
* Remove the `IL3050` dynamic-code warning from zero-based `Array2D`: `create` and `init` no longer allocate through a based-array path, and the `net10.0` asset allocates based arrays with `Array.CreateInstanceFromArrayType`. On that asset `createBased`, `initBased` and `zeroCreateBased` are annotated `RequiresDynamicCode`, so callers get `IL3050` at their own call site instead of a runtime failure — a new build error for AOT-analysing consumers that treat warnings as errors. ([Language Suggestion #1454](https://github.com/fsharp/fslang-suggestions/issues/1454), [PR #20338](https://github.com/dotnet/fsharp/pull/20338))
