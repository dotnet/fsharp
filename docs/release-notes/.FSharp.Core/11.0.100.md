...
* Remove the `IL3050` dynamic-code warning from `Array2D`: `create` and `init` no longer allocate through a based-array path, and the `net10.0` asset allocates based arrays with `Array.CreateInstanceFromArrayType`. ([Language Suggestion #1454](https://github.com/fsharp/fslang-suggestions/issues/1454), [PR #20338](https://github.com/dotnet/fsharp/pull/20338))
