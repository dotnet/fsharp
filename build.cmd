...
if not "%SIGN_TYPE%" == "" (
    ...
    echo %_msbuildexe% build\projects\Signing.proj /t:Build /p:SignType=%SIGN_TYPE% /p:Configuration=%BUILD_CONFIG% /p:ConfigFile=build\config\AssemblySignToolData.json
         %_msbuildexe% build\projects\Signing.proj /t:Build /p:SignType=%SIGN_TYPE% /p:Configuration=%BUILD_CONFIG% /p:ConfigFile=build\config\AssemblySignToolData.json
...
if not "%SIGN_TYPE%" == "" (
    echo %_msbuildexe% build\projects\Signing.proj /t:Build /p:SignType=%SIGN_TYPE% /p:Configuration=%BUILD_CONFIG% /p:ConfigFile=build\config\PackageSignToolData.json
         %_msbuildexe% build\projects\Signing.proj /t:Build /p:SignType=%SIGN_TYPE% /p:Configuration=%BUILD_CONFIG% /p:ConfigFile=build\config\PackageSignToolData.json
    if ERRORLEVEL 1 echo Error running sign tool && goto :failure
)
...
if not "%SIGN_TYPE%" == "" (
    echo %_msbuildexe% build\projects\Signing.proj /t:Build /p:SignType=%SIGN_TYPE% /p:Configuration=%BUILD_CONFIG% /p:ConfigFile=build\config\InsertionSignToolData.json
         %_msbuildexe% build\projects\Signing.proj /t:Build /p:SignType=%SIGN_TYPE% /p:Configuration=%BUILD_CONFIG% /p:ConfigFile=build\config\InsertionSignToolData.json
    if ERRORLEVEL 1 echo Error running sign tool && goto :failure
)
...
if "%TEST_NET40_FSHARP_SUITE%" == "1" (
    ...
    echo "%_dotnetexe%" test "%~dp0tests\fsharp\FSharp.Tests.FSharpSuite.fsproj" --no-restore --no-build -c %BUILD_CONFIG% -f net46 -l "trx;LogFileName=!LOGFILE!" -o "!FSCBINPATH!"
         "%_dotnetexe%" test "%~dp0tests\fsharp\FSharp.Tests.FSharpSuite.fsproj" --no-restore --no-build -c %BUILD_CONFIG% -f net46 -l "trx;LogFileName=!LOGFILE!" -o "!FSCBINPATH!"
...
if "%TEST_FCS%" == "1" (
    ...
    echo "!_dotnetexe!" test fcs/FSharp.Compiler.Service.Tests/FSharp.Compiler.Service.Tests.fsproj -c Release --logger:trx
         "!_dotnetexe!" test fcs/FSharp.Compiler.Service.Tests/FSharp.Compiler.Service.Tests.fsproj -c Release --logger:trx
...
if "%TEST_CORECLR_FSHARP_SUITE%" == "1" (
    ...
    echo "%_dotnetexe%" test "%~dp0tests\fsharp\FSharp.Tests.FSharpSuite.fsproj" --no-restore --no-build -c %BUILD_CONFIG% -f netcoreapp2.0 -l "trx;LogFileName=!LOGFILE!" -o "%BUILD_CONFIG%\coreclr\bin"
         "%_dotnetexe%" test "%~dp0tests\fsharp\FSharp.Tests.FSharpSuite.fsproj" --no-restore --no-build -c %BUILD_CONFIG% -f netcoreapp2.0 -l "trx;LogFileName=!LOGFILE!" -o "%BUILD_CONFIG%\coreclr\bin"
