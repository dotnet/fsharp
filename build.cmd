...
if "%BUILD_PROTO%" == "1" (
  ...
  if "%BUILD_PROTO_WITH_CORECLR_LKG%" == "1" (
    ...
    echo %_msbuildexe% %msbuildflags% src\fsharp-proto-build.proj /p:BUILD_PROTO_WITH_CORECLR_LKG=%BUILD_PROTO_WITH_CORECLR_LKG% /p:Configuration=Proto /p:DisableLocalization=true  /bl:%~dp0artifacts\protobuild-net40.build.binlog
         %_msbuildexe% %msbuildflags% src\fsharp-proto-build.proj /p:BUILD_PROTO_WITH_CORECLR_LKG=%BUILD_PROTO_WITH_CORECLR_LKG% /p:Configuration=Proto /p:DisableLocalization=true  /bl:%~dp0artifacts\protobuild-net40.build.binlog
  ...
  if "%BUILD_PROTO_WITH_CORECLR_LKG%" == "0" (
    ...
    echo %_msbuildexe% %msbuildflags% src\fsharp-proto-build.proj /p:BUILD_PROTO_WITH_CORECLR_LKG=%BUILD_PROTO_WITH_CORECLR_LKG% /p:Configuration=Proto /p:DisableLocalization=true  /bl:%~dp0artifacts\protobuild-coreclr.build.binlog
         %_msbuildexe% %msbuildflags% src\fsharp-proto-build.proj /p:BUILD_PROTO_WITH_CORECLR_LKG=%BUILD_PROTO_WITH_CORECLR_LKG% /p:Configuration=Proto /p:DisableLocalization=true  /bl:%~dp0artifacts\protobuild-coreclr.build.binlog
    @if ERRORLEVEL 1 echo Error: compiler proto build failed && goto :failure
  )
...
if "%BUILD_PHASE%" == "1" (
    ...
    echo %_msbuildexe% %msbuildflags% build-everything.proj /p:Configuration=%BUILD_CONFIG% %BUILD_DIAG% /p:BUILD_PUBLICSIGN=%BUILD_PUBLICSIGN%  /bl:%~dp0artifacts\msbuild.build-everything.build.%BUILD_CONFIG%.binlog
         %_msbuildexe% %msbuildflags% build-everything.proj /p:Configuration=%BUILD_CONFIG% %BUILD_DIAG% /p:BUILD_PUBLICSIGN=%BUILD_PUBLICSIGN%  /bl:%~dp0artifacts\msbuild.build-everything.build.%BUILD_CONFIG%.binlog
...
echo %_msbuildexe% %msbuildflags% build-nuget-packages.proj /p:Configuration=%BUILD_CONFIG%  /bl:=~dp0artifacts\msbuild.build-nuget-packages.build.%BUILD_CONFIG%.binlog
     %_msbuildexe% %msbuildflags% build-nuget-packages.proj /p:Configuration=%BUILD_CONFIG%  /bl:%~dp0artifacts\msbuild.build-nuget-packages.build.%BUILD_CONFIG%.binlog
...
if "%BUILD_SETUP%" == "1" (
    echo %_msbuildexe% %msbuildflags% setup\build-insertion.proj /p:Configuration=%BUILD_CONFIG%  /bl:%~dp0artifacts\msbuild.build-insertion.build.%BUILD_CONFIG%.binlog
         %_msbuildexe% %msbuildflags% setup\build-insertion.proj /p:Configuration=%BUILD_CONFIG%  /bl:%~dp0artifacts\msbuild.build-insertion.build.%BUILD_CONFIG%.binlog
...
if "%BUILD_SETUP%" == "1" (
    echo %_msbuildexe% %msbuildflags% setup\Swix\Microsoft.FSharp.vsmanproj /p:Configuration=%BUILD_CONFIG%  /bl:%~dp0artifacts\msbuild.setup-swix.build.%BUILD_CONFIG%.binlog
         %_msbuildexe% %msbuildflags% setup\Swix\Microsoft.FSharp.vsmanproj /p:Configuration=%BUILD_CONFIG%  /bl:%~dp0artifacts\msbuild.setup-swix.build.%BUILD_CONFIG%.binlog
