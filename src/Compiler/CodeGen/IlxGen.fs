...
type TypeDefBuilder(tdef: ILTypeDef, tdefDiscards) =
    ...
    member _.Close(g: TcGlobals) =
        ...
        //
        // However, raw-data static fields generated by GenConstArray (IlxGen.fs:~3115)
        // have the stable form "field<N>@" where N is the source-order counter from
        // mgbuf.GetOrAssignFieldCounter(m). Under --parallelcompilation+ these fields are
        // emitted from deferred method-body codegen — so their insertion order into
        // gfields races with other parallel emitters. Sort them by their numeric N to
        // restore byte-identical IL across sequential and parallel codegen. The numeric
        // suffix already encodes deterministic source order (assigned by
        // PrimeStableNamesForCodegen). Anchor them after all user fields so user struct
        // memory layout is unaffected. See https://github.com/dotnet/fsharp/issues/19928.
        let preservedFields =
            let isRawDataField (name: string) =
                name.Length > 6
                ...
                && (let mid = name.Substring(5, name.Length - 6)
                    ...
                    mid.Length > 0
                    && System.Linq.Enumerable.All(mid, fun (c: char) -> Char.IsDigit c))

            let parseRawDataN (name: string) =
                Int32.Parse(name.Substring(5, name.Length - 6))

            let userFields = ResizeArray<struct (string * int) * ILFieldDef>()
            let rawFields = ResizeArray<int * ILFieldDef>()

            for entry in gfields do
                let struct (name, _) = fst entry

                if isRawDataField name then
                    rawFields.Add(parseRawDataN name, snd entry)
                else
