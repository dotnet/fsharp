...
type AnonTypeGenerationTable() =
    ...
    let generateAnonType cenv (mgbuf: AssemblyBuilder) genToStringMethod (isStruct, ilTypeRef, nms) =
        ...
        if ilTypeRef.Scope.IsLocalRef then
            ...
            let ilCtorDef =
                (mkILSimpleStorageCtorWithParamNames (ilBaseTySpec, ilTy, [], flds, ILMemberAccess.Public, None, None)).With(
                    customAttrs = mkILCustomAttrs[GetDynamicDependencyAttribute g 0x660 ilTy])
                    ...
                    | TFSharpRecdRepr _ when not tycon.IsEnumTycon ->
                        ...
                        let ilMethodDef =
                            (mkILSimpleStorageCtorWithParamNames (
                                ...
                                customAttrs = mkILCustomAttrs[GetDynamicDependencyAttribute g 0x660 ilThisTy])
