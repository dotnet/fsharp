...
type TypeDefBuilder(tdef: ILTypeDef, tdefDiscards) =
    ...
    member _.Close(g: TcGlobals) =
        ...
        // Fields ordering:
        //   - User-defined fields (no '@' marker in name) preserve insertion order — for
        //     [<Struct>] DU/Record types the IL field declaration order determines physical
        //     memory layout.
        //   - Compiler-generated raw-data static fields ('field<line>_<hash>@') from
        //     GenConstArray are added by deferred parallel codegen in scheduler order. Their
        //     names are content-stable (encode source line + bytes hash), so we sort them by
        //     name to get SEQ=PAR determinism. Appending after user fields keeps user-field
        //     positions unchanged. See https://github.com/dotnet/fsharp/issues/19928.
        let preservedFields =
