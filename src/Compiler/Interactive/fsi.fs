...
type internal FsiDynamicCompiler(
                    ...
                    try
                        ...
                        if result.Success then
                            ...
                            if not (isNil scripts) then
                                fsiDynamicCompiler.EvalSourceFiles(ctok, istate, m, scripts, lexResourceManager, diagnosticsLogger, changeImplicitIncludeDir)
                            else istate
    ...
    member fsiDynamicCompiler.EvalSourceFiles(ctok, istate, m, sourceFiles, lexResourceManager, diagnosticsLogger: DiagnosticsLogger, changeImplicitIncludeDir) =
        let tcConfig = TcConfig.Create(tcConfigB,validate=false)
        match sourceFiles with
        | [] -> istate
        | _ ->
    ...
    let ExecInteraction (ctok, tcConfig:TcConfig, istate, action:ParsedScriptInteraction, diagnosticsLogger: DiagnosticsLogger) =
        ...
        istate |> InteractiveCatch diagnosticsLogger (fun istate ->
            ...
            | ParsedScriptInteraction.HashDirective (ParsedHashDirective("load", ParsedHashDirectiveArguments sourceFiles, m), _) ->
                let istate = fsiDynamicCompiler.CommitDependencyManagerText(ctok, istate, lexResourceManager, diagnosticsLogger)
                fsiDynamicCompiler.EvalSourceFiles (ctok, istate, m, sourceFiles, lexResourceManager, diagnosticsLogger, true),Completed None
            ...
            | (_,isScript1) :: _ ->
                ...
                let istate =
                    if isScript1 then
                        ...
                        istate |> InteractiveCatch diagnosticsLogger (fun istate -> fsiDynamicCompiler.EvalSourceFiles(ctok, istate, rangeStdin0, sourceFiles, lexResourceManager, diagnosticsLogger, true), Completed None) |> fst
                consume istate rest
