...
type public FSharpChecker =
    ...
    /// <param name="shareImportedAssemblies">Default: false. Indicates whether the contents imported from a referenced assembly may be shared between projects that resolve that assembly, and everything it can reach, to the same files. Saves memory and import time in solutions where projects reference the same binaries, and costs nothing where they do not. Assemblies containing type providers, and assemblies produced by a project in the same solution, are never shared.</param>
