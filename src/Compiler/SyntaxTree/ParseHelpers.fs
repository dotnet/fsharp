...
let mkAbstractMember parseState attrs (accessBeforeKeyword: SynAccess option) memberFlags (accessBeforeId: SynAccess option) mInline id typeParams typeWithConstraints accessors =
    ...
    let isInline, doc, explicitValTyparDecls = Option.isSome mInline, grabXmlDoc (parseState, attrs, 1), typeParams
    let mWith, (getSet, getSetRangeOpt: GetSetKeywords option, getterAccess, setterAccess) = accessors
    ...
    let trivia = { LeadingKeyword = leadingKeyword; InlineKeyword = mInline; WithKeyword = mWith; EqualsRange = None }
    let vis2 = SynValSigAccess.Single None
    let valSpfn = SynValSig(attrs, id, explicitValTyparDecls, ty, arity, isInline, false, doc, vis2, None, mWhole, trivia)
