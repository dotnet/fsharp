...
and TcConstExpr cenv (overallTy: OverallTy) env m tpenv c =
    ...
    | SynConst.UserNum (s, suffix) ->
        let expr =
            ...
            let ad = env.eAccessRights
            match ResolveLongIdentAsModuleOrNamespace cenv.tcSink cenv.amap m true OpenQualified env.eNameResEnv ad (ident (modName, m)) [] false true with
            | Result []
        ...
        | LongOrSingleIdent (false, SynLongIdent(longId, _, trivia), _, _) ->
            ...
            let resolvedToModuleOrNamespaceName =
                if delayed.IsEmpty then
                    ...
                    match ResolveLongIdentAsModuleOrNamespace cenv.tcSink cenv.amap m true OpenQualified env.eNameResEnv ad id rest true true with
