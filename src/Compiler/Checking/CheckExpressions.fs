...
type TcFileState =
    ...
    static member Create
        ...
        let instantiationGenerator m tpsorig traitCtxt = FreshenTypars traitCtxt m tpsorig
...
let CopyAndFixupTypars traitFreshner m rigid tpsorig =
    FreshenAndFixupTypars traitFreshner m rigid [] [] tpsorig
...
let MakeAndPublishVal (cenv: cenv) env (altActualParent, inSig, declKind, vrec, vscheme, attrs, doc, konst, isGeneratedEventVal) =
    ...
    let (ValScheme(id, typeScheme, topValInfo, memberInfoOpt, isMutable, inlineFlag, baseOrThis, vis, compgen, isIncrClass, isTyFunc, hasDeclaredTypars)) = vscheme
    ...
    let vspec =
        Construct.NewVal
            (logicalName, id.idRange, compiledName, ty, mut,
             compgen, topValInfo, vis, vrec, memberInfoOpt, baseOrThis, attrs, inlineFlag,
...
let AdjustAndForgetUsesOfRecValue cenv (vrefTgt: ValRef) (valScheme: ValScheme) =
    ...
    let valTy = GeneralizedTypeForTypeScheme valScheme.TypeScheme
...
let AdjustRecType (v: Val) vscheme =
    let (ValScheme(typeScheme=typeScheme; topValInfo=topValInfo)) = vscheme
    let valTy = GeneralizedTypeForTypeScheme typeScheme
    v.SetType valTy
    v.SetValReprInfo topValInfo
...
let FreshenPossibleForallTy traitFreshner g m rigid ty = 
    ...
    if isNil origTypars then
        ...
        let tps, renaming, tinst = CopyAndFixupTypars traitFreshner m rigid origTypars
...
let FreshenAbstractSlot traitFreshner g amap m synTyparDecls absMethInfo = 
    ...
    let typarsFromAbsSlot, typarInstFromAbsSlot, _ =
        ...
        FreshenAndFixupTypars traitFreshner m rigid ttps ttinst fmtps
...
let TcUnionCaseOrExnField cenv (env: TcEnv) ty1 m c n funcs =
    ...
    let mkf, argTys, _argNames =
      match ResolvePatternLongIdent cenv.tcSink cenv.nameResolver AllIdsOK false m ad env.TraitContext env.eNameResEnv TypeNameResolutionInfo.Default c with
...
let TcVal checkAttributes (cenv: cenv) (env: TcEnv) tpenv (vref: ValRef) optInst optAfterResolution m =
    let g = cenv.g
              ...
              if v.IsCtorThisVal && isRefCellTy g vTy then
                  ...
                  let tpsorig, vrefFlags, tinst, tau, tpenv =
                      ...
                      | Some(vrefFlags, checkTys) ->
                            ...
                            | ValInRecScope true
                            | ValNotInRecScope ->
                                let tpsorig, tps, tptys, tau = FreshenPossibleForallTy env.TraitContext g m TyparRigidity.Flexible vTy
...
let CompilePatternForMatch cenv (env: TcEnv) mExpr matchm warnOnUnused actionOnFailure (inputVal, generalizedTypars, inputExprOpt) clauses inputTy resultTy =
    let g = cenv.g
    let dtree, targets = CompilePattern g env.DisplayEnv cenv.amap (LightweightTcValForUsingInBuildMethodCall g env.TraitContext) cenv.infoReader mExpr matchm warnOnUnused actionOnFailure (inputVal, generalizedTypars, inputExprOpt) clauses inputTy resultTy
...
let AnalyzeArbitraryExprAsEnumerable (cenv: cenv) (env: TcEnv) localAlloc m exprTy expr =
    ...
    let tryType (exprToSearchForGetEnumeratorAndItem, tyToSearchForGetEnumeratorAndItem) =
        ...
        let getEnumerator_minst = FreshenMethInfo env.TraitContext m getEnumerator_minfo
        ...
        let moveNext_minst = FreshenMethInfo env.TraitContext m moveNext_minfo
        ...
        let get_Current_minst = FreshenMethInfo env.TraitContext m get_Current_minfo
...
and TcPatBindingName cenv env id ty isMemberThis vis1 topValInfo (inlineFlag, declaredTypars, argAttribs, isMutable, vis2, compgen) (names, takenNames: Set<string>) =
    ...
    let names = Map.add id.idText (PrelimValScheme1(id, declaredTypars, ty, topValInfo, None, isMutable, inlineFlag, baseOrThis, argAttribs, vis, compgen)) names
...
and IsNameOf (cenv: cenv) (env: TcEnv) ad m (id: Ident) =
    ...
    id.idText = "nameof" &&
    try
        match ResolveExprLongIdent cenv.tcSink cenv.nameResolver m ad env.TraitContext env.NameEnv TypeNameResolutionInfo.Default [id] with
...
let TcAndPublishValSpec (cenv, env, containerInfo: ContainerInfo, declKind, memFlagsOpt, tpenv, valSpfn) =
    ...
    (tpenv, valinfos) ||> List.mapFold (fun tpenv valSpecResult ->
            ...
            let paramNames =
                ...
                | None -> None
                | Some topValInfo -> Some topValInfo.ArgNames
