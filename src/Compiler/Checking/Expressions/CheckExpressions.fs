...
and TcCtorCall isNaked cenv env tpenv (overallTy: OverallTy) objTy mObjTyOpt item superInit args mWholeCall delayed afterTcOverloadResolutionOpt mItemIdentOpt =
    ...
    | Item.CtorGroup(methodName, minfos), _ ->
        ...
        TcMethodApplicationThen cenv env overallTy (Some objTy) tpenv None [] mWholeCall mItemIdent methodName ad PossiblyMutates false meths afterResolution isSuperInit args ExprAtomicFlag.NonAtomic None delayed
...
and TcMethodApplicationThen
       ...
       mItem       // The range of the method name identifier (narrowed for diagnostics, see #14284)
