...
and TcNormalizeReturnAttribs cenv env attrTgt attrs valSynData rtyOpt=
    let rotRetSynAttrs, rotRetAttribs, valAttribs =
        ...
        if List.length attribs <> List.length attrs then [], [], attribs
        else attribs
             |> List.zip attrs
...
and AnalyzeAndMakeAndPublishRecursiveValue
    ...
    let _, bindingAttribs, (SynValData(_, valSynInfo, _) as valSynData) = TcNormalizeReturnAttribs cenv env attrTgt bindingSynAttribs valSynData rtyOpt
