...
let private collectFullPathRefs (parsedInput: ParsedInput) : LongIdent list =
    ...
    let rec walkExpr (e: SynExpr) =
        ...
        | SynExpr.Quote(operator = op; quotedExpr = q) ->
            walkExpr op
            walkExpr q
        | SynExpr.Const _ -> ()
        | SynExpr.Typed(expr = e1; targetType = ty) ->
            walkExpr e1
            walkType ty
        ...
        | SynExpr.Record(baseInfo = baseInfo; copyInfo = copyInfo; recordFields = fields) ->
            ...
            for SynExprRecordField(fieldName = (SynLongIdent(id = ids), _); expr = eOpt) in fields do
                ...
                (match eOpt with
                 | Some e1 -> walkExpr e1
                 | None -> ())
        | SynExpr.New(targetType = ty; expr = e1) ->
            walkType ty
            walkExpr e1
        ...
        | SynExpr.Lambda(args = sps; body = e1) ->
            walkSimplePats sps
            walkExpr e1
        ...
        | SynExpr.DotNamedIndexedPropertySet(targetExpr = e1; argExpr = e2; rhsExpr = e3) ->
            ...
            walkExpr e2
            walkExpr e3
        | SynExpr.TypeTest(expr = e1; targetType = ty) ->
            walkExpr e1
            walkType ty
        | SynExpr.Upcast(expr = e1; targetType = ty) ->
            walkExpr e1
            walkType ty
        | SynExpr.Downcast(expr = e1; targetType = ty) ->
            walkExpr e1
            walkType ty
        ...
        | SynExpr.Dynamic(funcExpr = e1; argExpr = e2) ->
            walkExpr e1
            walkExpr e2
    ...
    and walkType (t: SynType) =
        ...
        | SynType.Fun(argType = a; returnType = r) ->
            walkType a
            walkType r
        | SynType.Var _ -> ()
        | SynType.Anon _ -> ()
        ...
        | SynType.StaticConstantNamed(ident = a; value = b) ->
            walkType a
            walkType b
        ...
        | SynType.Intersection(types = tys) ->
            for ty in tys do
                walkType ty
    ...
    and walkPat (p: SynPat) =
        ...
        | SynPat.Wild _ -> ()
        | SynPat.Named _ -> ()
        | SynPat.Typed(pat = sp; targetType = ty) ->
            walkPat sp
            walkType ty
        | SynPat.Attrib(pat = sp; attributes = attrs) ->
            walkAttribs attrs
            walkPat sp
        | SynPat.Or(lhsPat = a; rhsPat = b) ->
            walkPat a
            walkPat b
        | SynPat.ListCons(lhsPat = a; rhsPat = b) ->
            walkPat a
            walkPat b
        | SynPat.Ands(pats = ps) ->
            for sp in ps do
    ...
    and walkSimplePat (sp: SynSimplePat) =
        match sp with
        | SynSimplePat.Id _ -> ()
        | SynSimplePat.Typed(pat = inner; targetType = ty) ->
            walkSimplePat inner
            walkType ty
    ...
    and walkSimpleRepr (r: SynTypeDefnSimpleRepr) =
        ...
        | SynTypeDefnSimpleRepr.General(inherits = inhs; slotsigs = ss; fields = fields) ->
            for (ty, _, _) in inhs do
                walkType ty

            for (SynValSig(synType = ty), _) in ss do
                walkType ty
    ...
    let rec walkDecl (d: SynModuleDecl) =
        ...
        | SynModuleDecl.Types(typeDefns = tds) ->
            for td in tds do
                walkTypeDefn td
        | SynModuleDecl.Exception(
            exnDefn = SynExceptionDefn(exnRepr = SynExceptionDefnRepr(caseName = uc; attributes = attrs); members = ms)) ->
            walkAttribs attrs
            walkUnionCase uc
