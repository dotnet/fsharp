...
let ResolveLongIdentInType sink (ncenv: NameResolver) nenv lookupKind m ad traitCtxt id findFlag typeNameResInfo ty =
    let resInfo, item, rest =
        ResolveLongIdentInTypePrim ncenv nenv lookupKind ResolutionInfo.Empty 0 m ad traitCtxt id [] findFlag typeNameResInfo ty
        |> AtMostOneResult m
        |> ForceRaise
    ...
    | _ ->
        ...
        let tyconSearch =
            ...
            | id2 :: rest2 ->
                ...
                ResolveLongIdentInTyconRefs ResultCollectionSettings.AtMostOneResult ncenv nenv lookupKind (depth+1) m ad traitCtxt id2 rest2 typeNameResInfo id.idRange tcrefs
              ...
              let tyconSearch ad () =
                  ...
                  | id2 :: rest2 ->
                    ...
                    let tcrefs =
                       let typeNameResInfo = TypeNameResolutionInfo.ResolveToTypeRefs typeNameResInfo.StaticArgsInfo
                       CheckForTypeLegitimacyAndMultipleGenericTypeAmbiguities (tcrefs, typeNameResInfo, PermitDirectReferenceToGeneratedType.No, unionRanges m id.idRange)
                    ResolveLongIdentInTyconRefs ResultCollectionSettings.AtMostOneResult ncenv nenv lookupKind 1 m ad traitCtxt id2 rest2 typeNameResInfo id.idRange tcrefs
                  | _ ->
                    NoResultsOrUsefulErrors
...
let private ResolveExprDotLongIdent (ncenv: NameResolver) m ad traitCtxt nenv ty (id: Ident) rest (typeNameResInfo: TypeNameResolutionInfo) findFlag =
    ...
    let adhocDotSearchAccessible = AtMostOneResult m (ResolveLongIdentInTypePrim ncenv nenv lookupKind ResolutionInfo.Empty 1 m ad traitCtxt id rest findFlag typeNameResInfo ty)
    ...
    | Exception _ ->
        ...
        let adhocDotSearchAll () =
            ...
            ResolveLongIdentInTypePrim ncenv nenv lookupKind ResolutionInfo.Empty 1 m AccessibleFromSomeFSharpCode traitCtxt id rest findFlag typeNameResInfo ty
...
let ResolveExprDotLongIdentAndComputeRange (sink: TcResultsSink) (ncenv: NameResolver) wholem ad traitCtxt nenv ty lid (typeNameResInfo: TypeNameResolutionInfo) findFlag staticOnly =
    let resolveExpr findFlag =
        let resInfo, item, rest =
            match lid with
            | id :: rest ->
                ResolveExprDotLongIdent ncenv wholem ad traitCtxt nenv ty id rest typeNameResInfo findFlag
