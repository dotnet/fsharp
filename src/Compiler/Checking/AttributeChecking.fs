...
type AttribInfo = 
    ...
    member x.TyconRef = 
         ...
         | ILAttribInfo (g, amap, scoref, a, m) -> 
             let ty = RescopeAndImportILType scoref amap m [] a.Method.DeclaringType KnownAmbivalentToNull
             tcrefOfAppTy g ty
    ...
    member x.ConstructorArguments = 
         ...
         | ILAttribInfo (_g, amap, scoref, cattr, m) -> 
              ...
              [ for argTy, arg in Seq.zip cattr.Method.FormalArgTypes parms ->
                    let ty = RescopeAndImportILType scoref amap m [] argTy KnownAmbivalentToNull
