            ...
            | ScopeKind.TopModule ->
                if ctx.Pos.Line > 1 then
                    ...
                    let isImpliciteTopLevelModule =
                        not (line.StartsWith("module", StringComparison.Ordinal) && not (line.EndsWith("=", StringComparison.Ordinal)))
            ...
            | ScopeKind.Namespace ->
                ...
                if ctx.Pos.Line > 1 then
                    ...
                    |> List.tryPick (fun (i, lineStr) -> 
                        if lineStr.StartsWith("namespace", StringComparison.Ordinal) then Some i
                        else None)
                    |> function
