...
type internal TypeCheckInfo
    ...
    let GetEnvironmentLookupResolutions(nenv, ad, m, plid, filterCtors, showObsolete) =
        let items = ResolvePartialLongIdent ncenv nenv (ConstraintSolver.IsApplicableMethApprox g amap m traitCtxtNone) m ad plid showObsolete
        let items = items |> List.map ItemWithNoInst
        let items = items |> RemoveDuplicateItems g
        let items = items |> RemoveExplicitlySuppressed g
        let items = items |> FilterItemsForCtors filterCtors
...
type FSharpCheckProjectResults
    ...
    member _.GetOptimizedAssemblyContents() =
        ...
        let optimizedImpls, _optimizationData, _ = ApplyAllOptimizations (tcConfig, tcGlobals, LightweightTcValForUsingInBuildMethodCall tcGlobals traitCtxtNone, outfile, importMap, isIncrementalFragment, optEnv0, thisCcu, mimpls)
        let mimpls =
