...
type BackgroundCompiler(referenceResolver, projectCacheSize, keepAssemblyContents, keepAllBackgroundResolutions) as self =
            ...
            [ for (nm,opts) in options.ReferencedProjects ->
                { new IProjectReference with 
                        member x.EvaluateRawContents() = 
                            let r = self.ParseAndCheckProjectImpl(opts, ctok, CancellationToken.None)
                            r.RawFSharpAssemblyData 
                        member x.GetLogicalTimeStamp() = 
                            self.GetLogicalTimeStampForProject(ctok, opts, CancellationToken.None)
    ...
    member bc.ParseFileInProject(filename:string, source,options:FSharpProjectOptions) =
        ...
        | _ -> 
        reactor.EnqueueAndAwaitOpAsync("ParseFileInProject " + filename, fun ctok _ct -> 
    ...
    member bc.MatchBraces(filename:string, source, options)=
        reactor.EnqueueAndAwaitOpAsync("MatchBraces " + filename, fun ctok _ct -> 
...
//            | Some (parseResults, checkResults, _, _) when builder.AreCheckResultsBeforeFileInProjectReady(filename) -> 
    ...
    member bc.CheckFileInProject(parseResults: FSharpParseFileResults, filename, fileVersion, source, options, textSnapshotInfo) =
        ...
        async {
            let! builderOpt,creationErrors,_ = execWithReactorAsync <| fun ctok _ -> getOrCreateBuilder (ctok, options) // Q: Whis it it ok to ignore creationErrors in the build cache? A: These errors will be appended into the typecheck results
            use _unwind = IncrementalBuilder.KeepBuilderAlive builderOpt
            match builderOpt with
            ...
            | Some builder -> 
                ...
                | Some (_, checkResults) -> return FSharpCheckFileAnswer.Succeeded checkResults
                | _ ->
                    let! tcPrior = execWithReactorAsync <| fun ctok _ -> builder.GetCheckResultsBeforeFileInProject (ctok, filename)
    ...
    member private bc.GetLogicalTimeStampForProject(ctok, options, _ct) =
    ...
    member bc.KeepProjectAlive(options) =
        reactor.EnqueueAndAwaitOpAsync("KeepProjectAlive " + options.ProjectFileName, fun ctok _ct -> 
            let builderOpt,_creationErrors,_ = getOrCreateBuilder (ctok, options)
