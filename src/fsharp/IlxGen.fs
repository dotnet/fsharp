...
and GenApp cenv cgbuf eenv (f, fty, tyargs, curriedArgs, m) sequel =
  ...
  | (Expr.Val (v, _, _), _, _) when
    ...
    ->
        ...
        | BranchCallMethod (arityInfo, curriedArgInfos, _, numArgs, numObjArgs) ->
            ...
            let numArgs = 
                if v.IsExtensionMember then
                    ...
                    | [[]], [_] when numObjArgs = 0 -> (numArgs-1)
                    | [[_];[]], [_;_] when numObjArgs = 1 -> (numArgs-1)
                    | _ -> numArgs
                else numArgs
...
and GenObjectExpr cenv cgbuf eenvouter objExpr (baseType, baseValOpt, basecall, overrides, interfaceImpls, stateVars: ValRef list, m) sequel =
    ...
    // Note, the 'let' bindings for the stateVars have already been transformed to 'set' expressions, and thus the stateVars are now
    // free variables of the expression.
    let cloinfo, _, eenvinner = GetIlxClosureInfo cenv m ILBoxity.AsObject false [] eenvouter objExpr
...
and GenLambdaClosure cenv (cgbuf: CodeGenBuffer) eenv isLocalTypeFunc thisVars expr =
    ...
    | Expr.TyLambda (_, _, _, m, _) ->
        ...
        let cloinfo, body, eenvinner = GetIlxClosureInfo cenv m ILBoxity.AsObject isLocalTypeFunc thisVars eenv expr
...
and GetIlxClosureFreeVars cenv m (thisVars: ValRef list) boxity eenvouter takenNames expr =
    ...
    let ilCloAllFreeVars = Array.ofList ilCloFreeVars 
...
and GetIlxClosureInfo cenv m boxity isLocalTypeFunc thisVars eenvouter expr =
    let g = cenv.g
    let returnTy =
      match expr with
    ...
    let (cloAttribs, cloInternalFreeTyvars, cloContractFreeTyvars, _, cloFreeVars, ilCloTypeRef, ilCloAllFreeVars, eenvinner) =
...
and GenStaticOptimization cenv cgbuf eenv (constraints, e2, e3, _m) sequel =
    let e =
        if DecideStaticOptimizations cenv.g constraints = StaticOptimizationAnswer.Yes then
            e2
        else
            e3
    GenExpr cenv cgbuf eenv SPSuppress e sequel
...
and GenLetRecBindings cenv (cgbuf: CodeGenBuffer) eenv (allBinds: Bindings, m) =
    ...
    let computeFixupsForOneRecursiveVar boundv forwardReferenceSet fixups thisVars access set e =
        ...
        | Expr.Lambda _ | Expr.TyLambda _ | Expr.Obj _ ->
            ...
            let clo, _, eenvclo = GetIlxClosureInfo cenv m ILBoxity.AsObject isLocalTypeFunc thisVars {eenv with letBoundVars=(mkLocalValRef boundv) :: eenv.letBoundVars} e
...
and GenBindingAfterDebugPoint cenv cgbuf eenv sp (TBind(vspec, rhsExpr, _)) isStateVar startScopeMarkOpt =
    ...
    | Method (topValInfo, _, mspec, _, paramInfos, methodArgTys, retInfo) when not isStateVar ->
        ...
        generator cenv cgbuf.mgbuf eenv (vspec, mspec, access, paramInfos, retInfo, topValInfo, ctorThisValOpt, baseValOpt, tps, methodVars, methodArgTys, body', bodyty)
...
and GenParams (cenv: cenv) eenv (mspec: ILMethodSpec) (argInfos: ArgReprInfo list) methodArgTys (implValsOpt: Val list option) =
    let g = cenv.g
    let ilArgTys = mspec.FormalArgTypes

    let ilArgInfosAndTypes =
    ...
    let ilArgInfosAndTypes =
        ...
        | Some implVals when (implVals.Length = ilArgTys.Length) ->
            List.map2 (fun x y -> x, Some y) ilArgInfosAndTypes implVals
        | _ ->
            List.map (fun x -> x, None) ilArgInfosAndTypes

    let ilParams, _ = 
        (Set.empty, List.zip methodArgTys ilArgInfosAndTypes)
...
and GenMethodForBinding
    ...
    let tailCallInfo =
        [(mkLocalValRef v, BranchCallMethod (topValInfo.AritiesOfArgs, curriedArgInfos, methLambdaTypars, nonUnitMethodVars.Length, v.NumObjArgs))]
        ...
        if isUnitTy g ty && not v.IsMutable then Null, eenv
            ...
            | Some r when IsNamedLocalTypeFuncVal g v r ->
                ...
                let cloinfoGenerate eenv =
                    ...
                    let cloinfo, _, _ = GetIlxClosureInfo cenv v.Range ILBoxity.AsObject true [] eenvinner (Option.get repr)
                    cloinfo
