...
// This is used to determine if we can take the address of an immutable value in some
// situations.  It is not, however, correct for .NET struct types, which may be mutable, but we 
// are not aware if they are.  Thus, all .NET struct types are effectively assumed to be immutable.
let isDefinitelyMutable g ty =
    match tryDestAppTy g ty with 
    | None -> true
    | Some tcref -> isRecdOrUnionOrStructTyconRefDefinitelyMutable tcref

// Here, .NET struct types are assumed to be mutable, apart from two which are hardwired.  
// In these cases we give an optional warning when there is a discrepancy between this and the above.
let isPossiblyMutable g ty =
    match tryDestAppTy g ty with 
    | None -> true
    | Some tcref -> 
        isRecdOrUnionOrStructTyconRefDefinitelyMutable tcref ||
        (tcref.Deref.IsILStructTycon && not (tyconRefEq g tcref g.decimal_tcr) && not (tyconRefEq g tcref g.date_tcr))
...
let CanTakeAddressOf g m ty mut =
    ...
    | LikelyMutates
    | PossiblyMutates -> 
         // We don't take the address if it's definitely mutable
         // We take the address if it's not definitely mutable, but give a warning if it's possibly mutable
         let res = not (isDefinitelyMutable g ty)
         if isPossiblyMutable g ty && res then 
             if mut = LikelyMutates then 
                 warning(Error(FSComp.SR.tastLikelyMutationOfConstant(), m))
             warning(Error(FSComp.SR.tcStructTypeAssumedImmutable((tcrefOfAppTy g ty).DisplayName), m))
         res
