...
and 
    ...
    member x.Solution            = let sln = x.typar_solution in match box sln with null -> None | _ -> Some sln
...
let tryShortcutSolvedUnitPar canShortcut (r:Typar) = 
    ...
    | TType_measure unt -> 
        if canShortcut then 
            match unt with 
            | Measure.Var r2 -> 
               match box r2.typar_solution with 
               | null -> ()
               | _ -> 
                  r.typar_solution <- r2.typar_solution
            | _ -> () 
        unt
    | _ -> 
