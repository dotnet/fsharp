...
type ICache<'Key, 'Value when 'Key : equality and 'Value : not struct> =

    abstract Set: 'Key * 'Value -> unit

    abstract GetOrSet: 'Key * (unit -> 'Value) * ?isExistingKeyValid: ('Key -> bool) -> 'Value

    abstract TryGet: 'Key -> KeyValuePair<'Key, 'Value> voption

    abstract TryGetValue: 'Key -> 'Value voption

    abstract ContainsKey: 'Key -> bool

    abstract Remove: 'Key -> bool
...
/// Least recently refers to the eviction policy where we remove the least recently used items.
/// Thread safe.
[<Sealed>]
type LruCache<'Key, 'Value when 'Key : equality and 'Value : not struct> private (cacheSize: int, equalityComparer: IEqualityComparer<'Key>, onRemove) =
    ...
    let set key value =
        ...
        | _ ->
            if count = size then
                let lastPair = data.Last.Value
                assert (dataLookup.Remove lastPair.Key = true)
                data.RemoveLast ()
                onRemove lastPair
            else
                count <- count + 1
    ...
    interface ICache<'Key, 'Value> with
        ...
        member _.Set (key, value) =
            ...
            lock gate (fun () -> set key value)

        member _.GetOrSet (key, valueFactory, ?isExistingKeyValid) =
            if isKeyRef && obj.ReferenceEquals (key, null) then
                nullArg "key"

            if isValueRef && obj.ReferenceEquals (valueFactory, null) then
                nullArg "value"

            let isExistingKeyValid = defaultArg isExistingKeyValid (fun _ -> true)

            lock gate <| fun _ ->
                match dataLookup.TryGetValue key with
                | true, existingNode when isExistingKeyValid existingNode.Value.Key ->
            ...
            lock gate <| fun _ ->
                if count > 0 then
                    if keyEquals data.First.Value.Key key then
                        ValueSome data.First.Value
                    else
                        match dataLookup.TryGetValue key with
                        | true, existingNode ->
                            let existingPair = existingNode.Value
                            dataLookup.[existingPair.Key] <- data.AddFirst existingPair
                            data.Remove existingNode
                            onRemove existingPair
                            ValueSome existingPair
                        | _ ->
                            ValueNone
                else
                    ValueNone
        ...
        member x.TryGetValue key =
            (x :> ICache<'Key, 'Value>).TryGet key
            |> ValueOption.map (fun pair -> pair.Value)
    ...
    static member Create(cacheSize, equalityComparer, onRemove) =
        LruCache<'Key, 'Value>(cacheSize, equalityComparer, onRemove) :> ICache<'Key, 'Value>

/// Least recently used cache.
/// Least recently refers to the eviction policy where we remove the least recently used items.
/// Thread safe.
/// When an item has been removed, it is put into another LruCache that holds a WeakReference to the item.
[<Sealed>]
type LruWeakCache<'Key, 'Value when 'Key : equality and 'Value : not struct> private (strongCacheSize: int, weakCacheSize: int, equalityComparer: IEqualityComparer<'Key>) =
    
    let gate = obj ()
    let weakCache = LruCache<'Key, WeakReference<'Value>>.Create(weakCacheSize, equalityComparer, fun _ -> ())
    let cache = LruCache<'Key, 'Value>.Create(strongCacheSize, equalityComparer, fun pair -> weakCache.Set(pair.Key, WeakReference<_> pair.Value))

    let set key value =
        weakCache.Remove key |> ignore
        cache.Set(key, value)

    let getOrSet key valueFactory isExistingKeyValid =
        match cache.TryGet key with
        | ValueSome pair when isExistingKeyValid pair.Key ->
            pair.Value
        | _ ->
            match weakCache.TryGet key with
            | ValueSome pair when isExistingKeyValid pair.Key ->
                match pair.Value.TryGetTarget() with
                | true, value -> value
                | _ ->
                    assert (weakCache.Remove key = true)
                    let value = valueFactory ()
                    set key value
                    value
            | _ ->
    ...
    let tryGet key =
        match cache.TryGet key with
        | ValueSome pair -> ValueSome pair
        | _ ->
            match weakCache.TryGet key with
            | ValueSome pair ->
                let weakValue = pair.Value
                match weakValue.TryGetTarget () with
                | true, value -> 
                    set key value
                    ValueSome (KeyValuePair(pair.Key, value))
                | _ ->
                    weakCache.Remove key |> ignore
                    ValueNone
            | _ -> ValueNone
    ...
    interface ICache<'Key, 'Value> with
        ...
        member _.Set(key, value) =
            lock gate (fun _ -> set key value)

        member x.GetOrSet(key, valueFactory, ?isExistingKeyValid) =
            lock gate <| fun _ ->
                let isExistingKeyValid = defaultArg isExistingKeyValid (fun _ -> true)
                getOrSet key valueFactory isExistingKeyValid

        member _.TryGet key =
            lock gate (fun _ -> tryGet key)

        member x.TryGetValue key =
            (x :> ICache<'Key, 'Value>).TryGet key
            |> ValueOption.map (fun x -> x.Value)

        member _.ContainsKey key =
            cache.ContainsKey key || weakCache.ContainsKey key

        member _.Remove key =
            cache.Remove key || weakCache.Remove key

        member _.Count = cache.Count + weakCache.Count
    ...
    static member Create(strongCacheSize, weakCacheSize, equalityComparer) =
        LruWeakCache<'Key, 'Value>(strongCacheSize, weakCacheSize, equalityComparer) :> ICache<'Key, 'Value>
