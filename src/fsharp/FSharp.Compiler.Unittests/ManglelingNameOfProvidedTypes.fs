...
[<TestFixture>]
type ManglelingNamesOfProvidedTypesWithSingleParameter() =
    [<Test>]
    member this.MangleWithNonDefaultValue() = 
        let mangled = PrettyNaming.computeMangledNameWithoutDefaultArgValues("MyNamespace.Test",[|"xyz"|],[|"Foo",Some "abc"|])
        Assert.AreEqual("MyNamespace.Test,Foo=\"xyz\"",mangled)

    [<Test>]
    member this.MangleWithDefaultValue() = 
        let mangled = PrettyNaming.computeMangledNameWithoutDefaultArgValues("MyNamespace.Test",[|"xyz"|],[|"Foo",Some "xyz"|])
...
type ManglelingNamesOfProvidedTypesWithMultipleParameter() =
    ...
    [<Test>]
    member this.MangleWithNonDefaultValue() = 
        let mangled = PrettyNaming.computeMangledNameWithoutDefaultArgValues("MyNamespace.Test",[|"xyz"; "abc"|], [|"Foo",Some "foo"; "Foo2",Some "foo2"|])
        Assert.AreEqual("MyNamespace.Test,Foo=\"xyz\",Foo2=\"abc\"",mangled)

    [<Test>]
    member this.MangleWithDefaultValue() = 
        let mangled = PrettyNaming.computeMangledNameWithoutDefaultArgValues("MyNamespace.Test",[|"xyz"; "abc"|], [|"Foo",Some "xyz"; "Foo2",Some "abc"|])
        Assert.AreEqual("MyNamespace.Test,",mangled)
    ...
    member this.DemangleMultiParameter() = 
        ...
        Assert.AreEqual([|"Foo","xyz"; "Foo2","abc"|],parameters)