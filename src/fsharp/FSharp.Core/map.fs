...
namespace Microsoft.FSharp.Collections
    ...
    module MapTree = 
        ...
        let rec add (comparer: IComparer<'Value>) k v m = 
            match m with 
            | MapEmpty -> MapNode(k,v,MapEmpty,MapEmpty,1)            
            | MapNode(k2,v2,l,r,h) -> 
        ...
        let rec collapseLHS stack =
            ...
            | (MapNode(k,v,l,r,_)) :: rest -> collapseLHS (l :: MapNode (k,v,MapEmpty,MapEmpty,1) :: r :: rest)
        ...
        let rec moveNext i =
          if i.started then
            match i.stack with
              | MapNode _ :: rest -> i.stack <- collapseLHS rest;
                                     not i.stack.IsEmpty
              | [] -> false
