...
type CapturedNameResolution(p: pos, i: Item, tpinst, io: ItemOccurence, nre: NameResolutionEnv, ad: AccessorDomain, m: range) =
    member this.Pos = p
    member this.Item = i
            ...
            if allowedRange m then
                if replace then
                    ...
                    if not alreadyDone then
                        capturedNameResolutions.Add(CapturedNameResolution(endPos, item, tpinst, occurenceType, nenv, ad, m))
                        capturedMethodGroupResolutions.Add(CapturedNameResolution(endPos, itemMethodGroup, [], occurenceType, nenv, ad, m))
...
let CallNameResolutionSink (sink: TcResultsSink) (m: range, nenv, item, itemMethodGroup, tpinst, occurenceType, ad) =
    match sink.CurrentSink with
    | None -> ()
    | Some sink -> sink.NotifyNameResolution(m.End, item, itemMethodGroup, tpinst, occurenceType, nenv, ad, m, false)

let CallNameResolutionSinkReplacing (sink: TcResultsSink) (m: range, nenv, item, itemMethodGroup, tpinst, occurenceType, ad) =
    match sink.CurrentSink with
    | None -> ()
...
type ResolutionInfo =
    ...
    static member SendEntityPathToSink(sink, ncenv: NameResolver, nenv, occ, ad, ResolutionInfo(entityPath, warnings), typarChecker) =
        entityPath |> List.iter (fun (m, eref: EntityRef) ->
            ...
            CallNameResolutionSink sink (m, nenv, item, item, emptyTyparInst, occ, ad))
...
let rec ResolveLongIndentAsModuleOrNamespaceOrStaticClass sink (atMostOne: ResultCollectionSettings) (amap: Import.ImportMap) m allowStaticClasses first fullyQualified (nenv: NameResolutionEnv) ad (id:Ident) (rest: Ident list) isOpenDecl =
    ...
    if first && id.idText = MangledGlobalName then
        ...
        let notifyNameResolution (modref: ModuleOrNamespaceRef) m =
            ...
            CallNameResolutionSink sink (m, nenv, item, item, emptyTyparInst, occurence, ad)
...
let ResolveTypeLongIdentInTyconRef sink (ncenv: NameResolver) nenv typeNameResInfo ad m tcref (lid: Ident list) =
    ...
    CallNameResolutionSink sink (rangeOfLid lid, nenv, item, item, emptyTyparInst, ItemOccurence.UseInType, ad)
    tcref
    ...
    | id2 :: rest2 ->
        ...
        let modulSearch =
            ...
            | true, AccessibleEntityRef ncenv.amap m ad modref submodref ->
                ...
                CallNameResolutionSink sink (id.idRange, nenv, item, item, emptyTyparInst, ItemOccurence.Use, ad)
                let resInfo = resInfo.AddEntity(id.idRange, submodref)
...
let ResolveLongIdentAsExprAndComputeRange (sink: TcResultsSink) (ncenv: NameResolver) wholem ad nenv typeNameResInfo lid =
    ...
    let callSink (refinedItem, tpinst) =
        if not isFakeIdents then
            ...
            CallNameResolutionSink sink (itemRange, nenv, refinedItem, item, tpinst, occurence, ad)
    ...
    let afterResolution =
        ...
        | Some _ ->
            ...
            let callSink (refinedItem, tpinst) =
                ...
                CallNameResolutionSink sink (itemRange, nenv, refinedItem, unrefinedItem, tpinst, ItemOccurence.Use, ad)
