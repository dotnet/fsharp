...
type LoweredStateMachineFirstPhaseResult =
   {
     ...
     /// after the first phase. 
     ///
     phase2 : (Map<ILCodeLabel, int> -> Expr)
...
let (|StateMachine|_|) g expr =
    match expr with
    | ValApp g g.cgh_stateMachine_vref (_, [e], _m) -> Some e
    | _ -> None

let (|JumpTable|_|) g expr =
    match expr with
...
*)
    ...
    match overallExpr with
    | StateMachine g objExpr ->
        printfn "Found state machine..."
        match objExpr with 
        | Expr.Obj (_, ty, basev, basecall, overrides, interfaceImpls, _m) ->
            printfn "Found state machine object..."
            match overrides with 
            | [ (TObjExprMethod(slotsig, attribs, methTyparsOfOverridingMethod, methodParams, (JumpTable g (pcExpr, codeExpr)), m)) ] ->
                printfn "Found state machine override method..."
                // printfn "found seq { ... } or Seq.delay (fun () -> ...) in FSharp.Core.dll"
                //let m = e.Range
                let initLabel = IL.generateCodeLabel()
                let noDisposeContinuationLabel = IL.generateCodeLabel()
                ...
                | Some res ->
                    ...
                    // Execute phase2, building the core of the the GenerateNext, Dispose and CheckDispose methods
                    let methodBodyExprR = res.phase2 lab2pc
                    let methodBodyExprWithJumpTable = addJumpTable pcExpr methodBodyExprR
                    let overrideR = TObjExprMethod(slotsig, attribs, methTyparsOfOverridingMethod, methodParams, methodBodyExprWithJumpTable, m) 
                    Some (objExpr, ty, basev, basecall, [overrideR], interfaceImpls, stateVars, m)
                | None -> None
            | _ -> None
        | _ -> None
    | _ -> None
