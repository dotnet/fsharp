                ...
                | Result (_, item, _, _, _ as res)
                    when
                         (match item with
                          ...
                          | _ -> true) ->
                    let overallTy = match overallTyOpt with None -> MustEqual (NewInferenceType cenv.g) | Some t -> t
...
and TcMethodApplicationThen
    ...
    let exprTy = if isNil delayed then overallTy else MustEqual (NewInferenceType cenv.g)
...
and GetNewInferenceTypeForMethodArg cenv env tpenv x =
    ...
    | SynExpr.Lambda (body = a) -> mkFunTy cenv.g (NewInferenceType cenv.g) (GetNewInferenceTypeForMethodArg cenv env tpenv a)
...
and TcMethodApplication
    ...
    let curriedCallerArgs, exprTy, delayed =
        ...
        | [calledMeth] when not isProp && calledMeth.NumArgs.Length > 1 ->
            [], MustEqual (NewInferenceType cenv.g), [ for x in curriedCallerArgs -> DelayedApp(ExprAtomicFlag.NonAtomic, false, None, x, x.Range) ] @ delayed
