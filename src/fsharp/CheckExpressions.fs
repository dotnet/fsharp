...
and TcExprOfUnknownTypeThen cenv env tpenv expr delayed =
    ...
    let expr', tpenv =
      try
        TcExprThen cenv exprty env tpenv false expr delayed
      with e ->
        let m = expr.Range
        errorRecovery e m
...
// Check seq { expr }
// Check async { expr }
and TcApplicationThen cenv overallTy env tpenv mExprAndArg synLeftExprOpt leftExpr exprty (synArg: SynExpr) atomicFlag isSugar delayed =
    let denv = env.DisplayEnv
    let mArg = synArg.Range
    ...
    | ValueNone ->
        ...
        | _ ->
            error (NotAFunction(denv, overallTy, mLeftExpr, mArg))
