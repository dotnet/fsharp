...
let TcConst cenv ty m env c =
    ...
    | SynConst.String (s, _, _, _) -> unif cenv.g.string_ty; Const.String s
...
and TcStaticConstantParameter cenv (env: TcEnv) tpenv kind (StripParenTypes v) idOpt container =
    ...
    | SynType.StaticConstant(sc, _) ->
        let v =
            ...
            | SynConst.String (s, _, _, _) when s <> null && typeEquiv g g.string_ty kind -> record(g.string_ty); box (s: string)
...
and TcExprUndelayed cenv overallTy env tpenv (synExpr: SynExpr) =
    ...
    | SynExpr.Const (SynConst.String (s, _, _, m), _) -> 
            ...
            | Result ((_, mref, _) :: _) -> 
                let expr = 
                    ...
                    with _ -> 
                        ...
                        with _ ->             
                            SynExpr.App (ExprAtomicFlag.Atomic, false, mkSynLidGet m [modName] "FromString", SynExpr.Const (SynConst.String (s, false, false, m), m), m) 
