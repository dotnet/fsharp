...
type CapabilitiesManager(config: FSharpLanguageServerConfig, scOverrides: IServerCapabilitiesOverride seq) =
    ...
    let defaultCapabilities (clientCapabilities: ClientCapabilities) =
        ...
        ServerCapabilities(
            ...
            DiagnosticOptions =
                addIf config.EnabledFeatures.Diagnostics (DiagnosticOptions(WorkDoneProgress = true, InterFileDependencies = true, Identifier = "potato", WorkspaceDiagnostics = true)),
            ...
            SemanticTokensOptions =
                addIf config.EnabledFeatures.SemanticHighlighting
