...
if Foo.ReadOnly1 <> 1           then exit 1
if lastUsed <> ("ReadOnly1", 0) then exit 1

if Foo.ReadOnly2 <> 2           then exit 2
if lastUsed <> ("ReadOnly2", 0) then exit 2

Foo.WriteOnly <- -123
if lastUsed <> ("WriteOnly", -123) then exit 3

if Foo.ReadWrite1 <> 3           then exit 4
if lastUsed <> ("ReadWrite1", 0) then exit 4

Foo.ReadWrite1 <- -456
if lastUsed <> ("ReadWrite1", -456) then exit 4

if Foo.ReadWrite2 <> 4           then exit 5
if lastUsed <> ("ReadWrite2", 0) then exit 5

Foo.ReadWrite2 <- -789
if lastUsed <> ("ReadWrite2", -789) then exit 5
