//<Expects id="FS1246" span="(9,19-9,48)" status="error">'CallerMemberName' must be applied to an argument of type 'string', but has been applied to an argument of type 'int'</Expects>
...
type MyTy() =
    static member GetCallerMemberNamehNotString([<CallerMemberName>] ?name : int) =
        name
