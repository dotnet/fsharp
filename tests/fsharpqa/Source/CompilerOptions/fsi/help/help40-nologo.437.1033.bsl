...
--compilertool:<file>                    Reference an assembly or directory containing a design time tool (Short
                                         form: -t)
--usesdkrefs[+|-]                        Use reference assemblies for .NET framework references when available
                                         (Enabled by default).
-- ...                                   Treat remaining arguments as command line arguments, accessed using
                                         fsi.CommandLineArgs


                - CODE GENERATION -
--debug[+|-]                             Emit debug information (Short form: -g) (on by default)
--debug:{full|pdbonly|portable|embedded} Specify debugging type: full, portable, embedded, pdbonly. ('pdbonly' is the
...
--typecheck-only                         Perform type checking only, do not execute code
--exec                                   Exit fsi after loading the files or running the .fsx script given on the
                                         command line
...
--shadowcopyreferences[+|-]              Prevents references from being locked by the F# Interactive process (off by
                                         default)
