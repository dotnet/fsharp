...
let config configurationName envVars =
...
#if NET462
    let fscArchitecture = "net462"
    let fsiArchitecture = "net462"
    let fsharpCoreArchitecture = "net45"
    let fsharpBuildArchitecture = "net462"
    let fsharpCompilerInteractiveSettingsArchitecture = "net462"
#else
    ...
    let PEVERIFY = requireFile (artifactsBinPath ++ "PEVerify" ++ configurationName ++ "net462" ++ "PEVerify.exe")
...
#if !FSHARP_SUITE_DRIVES_CORECLR_TESTS
    let FSIANYCPU = requireFile (artifactsBinPath ++ "fsiAnyCpu" ++ configurationName ++ "net462" ++ "fsiAnyCpu.exe")
#endif
