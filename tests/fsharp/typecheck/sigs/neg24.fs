...
module Positive2 = 
    ...
    //let v5 = [ if true then 1 ] // "This 'if' expression is missing an 'else' branch. Because 'if' is an expression..."
    let a3 = [| (if true then 1 else 2) |] 
    let a4 = [| if true then 1 else yield 2 |] 
    //let a5 = [| if true then 1 |] // "This 'if' expression is missing an 'else' branch. Because 'if' is an expression..." 
    let s2 = seq { if true then () else () } 
    let s6 = seq { if true then () } 
    ...
    //let l9 = [ printfn "hello"; 1; 2 ] // "All elements of a list must be of the same type as the first element, which here is 'unit'. This element..."
    let l10 = [ printfn "hello"; yield 1; yield 2 ] // expect ok
