...
neg31.fs(32,6,32,82): typecheck error FS3888: The attribute 'Obsolete' is present on 'C3' in the implementation but not in the signature. Code that consumes this signature depends on this attribute, so add it to the signature. Tooling skips the implementation when a signature file is present.
...
neg31.fs(52,6,52,82): typecheck error FS3888: The attribute 'Obsolete' is present on 'M3' in the implementation but not in the signature. Code that consumes this signature depends on this attribute, so add it to the signature. Tooling skips the implementation when a signature file is present.
