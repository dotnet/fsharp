...
type TaskSeqBuilder() =
    ...
    //        let __stack_step = __expand_body ()
    //        step <- __stack_step
    //    step
    ...
    member inline __.TryWith(__expand_body : TaskSeqCode<'T>, __expand_catch : exn -> TaskSeqCode<'T>) : TaskSeqCode<'T> =
        (fun sm -> 
            ...
            try
                let __stack_step = __expand_body sm
                step <- __stack_step
                // TODO: If we are not doing state machine compilation, then if "__stack_step" is false we must adjust the 
