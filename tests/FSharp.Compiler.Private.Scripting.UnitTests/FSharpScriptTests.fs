...
x
    ...
    member _.``Capture console output/error``() =
        ...
        script.Console.OutputProduced.Add (fun line -> if line = "stdout:1234" then sawOutputSentinel.Set() |> ignore)
        script.Console.ErrorProduced.Add (fun line -> if line = "stderr:5678" then sawErrorSentinel.Set() |> ignore)
...
"
    ...
    member _.``Eval script with invalid PackageName should fail immediately``() =
        ...
        script.Console.OutputProduced.Add(
            fun line ->
    ...
    member _.``Eval script with invalid PackageName should fail immediately and resolve one time only``() =
        ...
        script.Console.OutputProduced.Add (fun line -> if line.Contains("error NU1101:") then foundResolve <- foundResolve + 1)
        let result, errors =
            script.Eval("""
