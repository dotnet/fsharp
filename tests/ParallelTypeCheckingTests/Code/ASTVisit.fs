...
type Reference =
    { Ident: SimpleId; Kind: ReferenceKind }
...
type ReferenceSimple =
    { Ident: string[]; Kind: ReferenceKind }
...
module ASTVisit =
    ...
    let rec visitSynModuleDecl (decl: SynModuleDecl) : References =
        // TODO
        match decl with
        | SynModuleDecl.Attributes (synAttributeLists, range) -> visitSynAttributeLists synAttributeLists
        | SynModuleDecl.Exception (synExceptionDefn, range) -> visitSynExceptionDefn synExceptionDefn
        | SynModuleDecl.Expr (synExpr, range) -> visitExpr synExpr
        | SynModuleDecl.HashDirective (parsedHashDirective, range) -> visitHashDirective parsedHashDirective
        | SynModuleDecl.Let (isRecursive, synBindings, range) -> visitBindings synBindings
        | SynModuleDecl.Open (synOpenDeclTarget, range) -> visitSynOpenDeclTarget synOpenDeclTarget
        | SynModuleDecl.Types (synTypeDefns, range) -> visitSynTypeDefns synTypeDefns
        | SynModuleDecl.ModuleAbbrev (ident, longId, range) -> [ ReferenceOrAbbreviation.Abbreviation Abbreviation.ModuleAbbreviation ]
        | SynModuleDecl.NamespaceFragment synModuleOrNamespace -> visitSynModuleOrNamespace synModuleOrNamespace
    ...
    and visitSynModuleDeclNestedModuleTrivia (x: SynModuleDeclNestedModuleTrivia) : References = [] // TODO check

    and visitHashDirective (x: ParsedHashDirective) : References = [] // TODO Check

    and visitSynIdent (x: SynIdent) : References = [] // TODO Check
    ...
    and visitSynTupleTypeSegment (x: SynTupleTypeSegment) : References =
        ...
        | SynTupleTypeSegment.Type typeName -> visitType typeName

    and visitSynTupleTypeSegments (x: SynTupleTypeSegment list) : References = Seq.collect visitSynTupleTypeSegment x

    and visitTypar (x: SynTypar) : References =
        match x with
        | SynTypar.SynTypar (ident, typarStaticReq, isCompGen) -> [] // TODO check

    and visitSynRationalConst (x: SynRationalConst) = [] // TODO check

    and visitSynConst (x: SynConst) : References = [] // TODO Check

    and visitSynTypes (x: SynType list) : References = Seq.collect visitType x

    and visitTypeConstraints (x: SynTypeConstraint list) : References = Seq.collect visitTypeConstraint x
    ...
    and visitSynTyparDeclList (x: SynTyparDecl list) : References = Seq.collect visitSynTyparDecl x
    ...
    and visitSynTyparDecls (x: SynTyparDecls) : References =
        ...
        | SynTyparDecls.PrefixList (synTyparDecls, range) -> visitSynTyparDeclList synTyparDecls
        | SynTyparDecls.SinglePrefix (synTyparDecl, range) -> visitSynTyparDecl synTyparDecl
    ...
    and visitValSig (x: SynValSig) : References =
        ...
        | SynValSig (synAttributeLists,
            ...
            seq {
                ...
                match synAccessOption with
                | Some access -> yield! visitSynAccess access
                | None -> ()

                match synExprOption with
                | Some expr -> yield! visitExpr expr
                | None -> ()
            }

    and visitSynTypeDefnSignRepr (x: SynTypeDefnSigRepr) : References =
        match x with
        | SynTypeDefnSigRepr.Exception synExceptionDefnRepr -> visitSynExceptionDefnRepr synExceptionDefnRepr
        | SynTypeDefnSigRepr.Simple (synTypeDefnSimpleRepr, range) -> visitTypeDefnSimpleRepr synTypeDefnSimpleRepr
    ...
    and visitMemberSig (x: SynMemberSig) : References =
        match x with
        | SynMemberSig.Inherit (inheritedType, range) -> visitType inheritedType
        | SynMemberSig.Interface (interfaceType, range) -> visitType interfaceType
        | SynMemberSig.Member (synValSig, synMemberFlags, range) -> seq { yield! visitValSig synValSig }
        | SynMemberSig.NestedType (synTypeDefnSig, range) -> visitSynTypeDefnSign synTypeDefnSig
        | SynMemberSig.ValField (synField, range) -> visitSynField synField

    and visitTypeConstraint (x: SynTypeConstraint) : References =
        match x with
        | SynTypeConstraint.WhereTyparIsValueType (typar, range) -> visitTypar typar
        | SynTypeConstraint.WhereTyparIsReferenceType (typar, range) -> visitTypar typar
        | SynTypeConstraint.WhereTyparIsUnmanaged (typar, range) -> visitTypar typar
        | SynTypeConstraint.WhereTyparSupportsNull (typar, range) -> visitTypar typar
        | SynTypeConstraint.WhereTyparIsComparable (typar, range) -> visitTypar typar
        | SynTypeConstraint.WhereTyparIsEquatable (typar, range) -> visitTypar typar
        ...
        | SynTypeConstraint.WhereSelfConstrained (selfConstraint, range) -> visitType selfConstraint
    ...
    and visitType (x: SynType) : References =
        ...
        | SynType.Array (rank, elementType, range) -> visitType elementType
        ...
        | SynType.Paren (innerType, range) -> visitType innerType
        | SynType.Tuple (isStruct, synTupleTypeSegments, range) -> visitSynTupleTypeSegments synTupleTypeSegments
        | SynType.Var (synTypar, range) -> visitTypar synTypar
        | SynType.AnonRecd (isStruct, fields, range) -> fields |> Seq.collect (fun (id, f) -> visitType f)
        | SynType.HashConstraint (innerType, range) -> visitType innerType
        | SynType.LongIdent synLongIdent -> visitSynLongIdent synLongIdent
        ...
        | SynType.StaticConstant (synConst, range) -> visitSynConst synConst
        ...
        | SynType.StaticConstantExpr (synExpr, range) -> seq { yield! visitExpr synExpr }
    ...
    and visitPreXmlDoc (doc: FSharp.Compiler.Xml.PreXmlDoc) : References = [] // TODO Check

    and visitSynAccess (x: SynAccess) : References = [] // TODO check

    and visitSynField (x: SynField) : References =
        match x with
        | SynField.SynField (synAttributeLists,
                             isStatic,
                             identOption,
                             fieldType,
                             isMutable,
    ...
    and visitSynFields (x: SynField list) : References = Seq.collect visitSynField x
    ...
    and visitSynUnionCaseKind (x: SynUnionCaseKind) : References =
        ...
        | SynUnionCaseKind.Fields synFields ->
            ...
            [] // TODO
        | SynUnionCaseKind.FullType (fullType, fullTypeInfo) -> [] // TODO
    ...
    and visitSynExceptionDefnRepr (x: SynExceptionDefnRepr) : References =
        ...
        | SynExceptionDefnRepr.SynExceptionDefnRepr (synAttributeLists, synUnionCase, identsOption, preXmlDoc, synAccessOption, range) ->
            seq {
                ...
                match identsOption with
                | Some ident -> yield! visitLongIdent ident
                | None -> ()

                yield! visitPreXmlDoc preXmlDoc

                match synAccessOption with
                | Some synAccess -> yield! visitSynAccess synAccess
                | None -> ()
            }
    ...
    and visitTypeDefnSimpleRepr (x: SynTypeDefnSimpleRepr) : References =
        match x with
        | SynTypeDefnSimpleRepr.Enum (synEnumCases, range) -> visitEnumCases synEnumCases
        | SynTypeDefnSimpleRepr.Exception synExceptionDefnRepr -> visitSynExceptionDefnRepr synExceptionDefnRepr
    ...
    and visitSynArgInfo (x: SynArgInfo) : References =
        match x with
        | SynArgInfo (synAttributeLists, optional, identOption) -> visitSynAttributeLists synAttributeLists
    ...
    and visitSynTypeDefnRepr (x: SynTypeDefnRepr) : References =
        match x with
        | SynTypeDefnRepr.Exception synExceptionDefnRepr -> visitSynExceptionDefnRepr synExceptionDefnRepr
        | SynTypeDefnRepr.Simple (synTypeDefnSimpleRepr, range) -> visitTypeDefnSimpleRepr synTypeDefnSimpleRepr
    ...
    and visitSynSimplePats (x: SynSimplePats) : References =
        ...
        | SynSimplePats.SimplePats (synSimplePats, range) -> Seq.collect visitSynSimplePat synSimplePats

    and visitSynSimplePatAlternativeIdInfoRef (x: SynSimplePatAlternativeIdInfo ref) : References = [] // TODO Check
    ...
    and visitSynSimplePat (x: SynSimplePat) : References =
        ...
        | SynSimplePat.Id (ident, synSimplePatAlternativeIdInfoRefOption, isCompilerGenerated, isThisVal, isOptional, range) ->
            seq {
                ...
                | Some info -> yield! visitSynSimplePatAlternativeIdInfoRef info
                | None -> ()
            }
    ...
    and visitSynMemberDefn (defn: SynMemberDefn) : References =
        match defn with
        | SynMemberDefn.Inherit (baseType, identOption, range) -> visitType baseType
        | SynMemberDefn.Interface (interfaceType, withKeyword, synMemberDefnsOption, range) ->
            seq {
                ...
                | Some defns -> yield! visitSynMemberDefns defns
                | None -> ()
            }
        | SynMemberDefn.Member (memberDefn, range) -> visitSynBinding memberDefn
        | SynMemberDefn.Open (synOpenDeclTarget, range) -> visitSynOpenDeclTarget synOpenDeclTarget
        ...
        | SynMemberDefn.AutoProperty (synAttributeLists,
                                      isStatic,
                                      ident,
                                      synTypeOption,
                                      synMemberKind,
            ...
            seq {
                ...
                match synTypeOption with
                | Some synType -> yield! visitType synType
                | None -> ()
        ...
        | SynMemberDefn.LetBindings (synBindings, isStatic, isRecursive, range) -> visitBindings synBindings
        ...
        | SynMemberDefn.ValField (fieldInfo, range) -> visitSynField fieldInfo
        | SynMemberDefn.GetSetMember (memberDefnForGet, memberDefnForSet, range, synMemberGetSetTrivia) ->
            seq {
                match memberDefnForGet with
                | Some binding -> yield! visitSynBinding binding
                | None -> ()

                match memberDefnForSet with
                | Some binding -> yield! visitSynBinding binding
                | None -> ()
            }

    and visitSynMemberDefns (defns: SynMemberDefn list) : References = Seq.collect visitSynMemberDefn defns
    ...
    and visitSynTypeDefn (defn: SynTypeDefn) : References =
        ...
        | SynTypeDefn.SynTypeDefn (synComponentInfo, synTypeDefnRepr, synMemberDefns, synMemberDefnOption, range, synTypeDefnTrivia) ->
            seq {
                ...
                | Some defn -> yield! visitSynMemberDefn defn
                | None -> ()
            }

    and visitSynTypeDefns (defns: SynTypeDefn list) : References = Seq.collect visitSynTypeDefn defns
    ...
    and visitSynValData (x: SynValData) : References =
        ...
        | SynValData (synMemberFlagsOption, synValInfo, thisIdOpt) ->
            seq {
                match synMemberFlagsOption with
                | Some flags -> yield! visitSynMemberFlags flags
                | None -> ()

                yield! visitSynValInfo synValInfo
            }

    and visitSynPats (x: SynPat list) : References = Seq.collect visitPat x

    and visitPat (x: SynPat) : References =
        match x with
        | SynPat.Ands (synPats, range) -> visitSynPats synPats
        | SynPat.As (lhsPat, rhsPat, range) -> visitSynPats [ lhsPat; rhsPat ]
        | SynPat.Attrib (synPat, synAttributeLists, range) ->
            seq {
                yield! visitPat synPat
                yield! visitSynAttributeLists synAttributeLists
            }
        | SynPat.Const (synConst, range) -> visitSynConst synConst
        ...
        | SynPat.Null range -> []
        | SynPat.Or (lhsPat, rhsPat, range, synPatOrTrivia) -> seq { yield! visitSynPats [ lhsPat; rhsPat ] }
        | SynPat.Paren (synPat, range) -> visitPat synPat
        ...
        | SynPat.Tuple (isStruct, elementPats, range) -> visitSynPats elementPats
        ...
        | SynPat.IsInst (synType, range) -> visitType synType
        ...
        | SynPat.LongIdent (synLongIdent, identOption, synValTyparDeclsOption, synArgPats, synAccessOption, range) ->
            seq {
                ...
                match synValTyparDeclsOption with
                | Some decls -> yield! visitSynValTyparDecls decls
                | None -> ()
        ...
        | SynPat.QuoteExpr (synExpr, range) -> visitExpr synExpr
        | SynPat.ArrayOrList (isArray, elementPats, range) -> visitSynPats elementPats
        | SynPat.DeprecatedCharRange (startChar, endChar, range) -> []
        | SynPat.FromParseError (synPat, range) -> visitPat synPat
    ...
    and visitSynBinding (x: SynBinding) : References =
        match x with
        | SynBinding.SynBinding (synAccessOption,
                                 synBindingKind,
                                 isInline,
                                 isMutable,
                                 synAttributeLists,
            ...
            seq {
                ...
                | Some info -> yield! visitBindingReturnInfo info
                | None -> ()

                yield! visitExpr synExpr
            }

    and visitBindings (bindings: SynBinding list) : References = Seq.collect visitSynBinding bindings

    and visitSynOpenDeclTarget (target: SynOpenDeclTarget) : References =
        match target with
        | SynOpenDeclTarget.Type (typeName, range) -> visitType typeName
        | SynOpenDeclTarget.ModuleOrNamespace (synLongIdent, range) ->
            [
                ReferenceOrAbbreviation.Reference
                    {
                        Ident = synLongIdent.LongIdent |> extractSimpleId
    ...
    and visitSynComponentInfo (info: SynComponentInfo) : References =
        ...
        | SynComponentInfo (synAttributeLists,
            ...
            seq {
                ...
                match synTyparDeclsOption with
                | Some decls -> yield! visitSynTyparDecls decls
                | None -> ()
    ...
    and visitSynLongIdent (ident: SynLongIdent) : References =
        [
            ReferenceOrAbbreviation.Reference
                {
                    Ident = ident.LongIdent |> extractSimpleId
                    Kind = ReferenceKind.Type
                }
        ]
    ...
    and visitSynInterpolatedStringPart (x: SynInterpolatedStringPart) : References =
        ...
        | SynInterpolatedStringPart.FillExpr (fillExpr, identOption) -> visitExpr fillExpr
    ...
    and visitSynInterfaceImpls (x: SynInterfaceImpl list) : References = Seq.collect visitSynInterfaceImpl x

    and visitSynArgPats (x: SynArgPats) : References =
        match x with
        | SynArgPats.Pats synPats -> visitSynPats synPats
        | SynArgPats.NamePatPairs (tuples, range, synArgPatsNamePatPairsTrivia) ->
            tuples |> Seq.collect (fun (ident, range, pat) -> visitPat pat)

    and visitSynMemberSig (x: SynMemberSig) : References =
        match x with
        | SynMemberSig.Inherit (inheritedType, range) -> visitType inheritedType
        | SynMemberSig.Interface (interfaceType, range) -> visitType interfaceType
        | SynMemberSig.Member (synValSig, synMemberFlags, range) -> visitValSig synValSig
        | SynMemberSig.NestedType (synTypeDefnSig, range) -> visitSynTypeDefnSign synTypeDefnSig
        | SynMemberSig.ValField (synField, range) -> visitSynField synField

    and visitExpr (expr: SynExpr) =
        let l = System.Collections.Generic.List<ReferenceOrAbbreviation>()
        let go (items: References) = l.AddRange(items)

        match expr with
        | SynExpr.App (exprAtomicFlag, isInfix, funcExpr, argExpr, range) ->
            visitExpr funcExpr |> go
            visitExpr argExpr |> go
        | SynExpr.Assert (synExpr, range) -> visitExpr synExpr |> go
        | SynExpr.Const (synConst, range) -> visitSynConst synConst |> go
        | SynExpr.Do (synExpr, range) -> visitExpr synExpr |> go
        ...
        | SynExpr.Dynamic (funcExpr, qmark, argExpr, range) ->
            visitExpr funcExpr |> go
            visitExpr argExpr |> go
        | SynExpr.Fixed (synExpr, range) -> visitExpr synExpr |> go
        ...
        | SynExpr.Lambda (fromMethod, inLambdaSeq, synSimplePats, synExpr, tupleOption, range, synExprLambdaTrivia) ->
            ...
            | Some (simplePats, expr) ->
                visitSynPats simplePats |> go
                visitExpr expr |> go
            | None -> ()
        | SynExpr.Lazy (synExpr, range) -> visitExpr synExpr |> go
        ...
        | SynExpr.New (isProtected, targetType, synExpr, range) ->
            ...
            visitExpr synExpr |> go
        | SynExpr.Null range -> ()
        | SynExpr.Paren (synExpr, leftParenRange, rightParenRange, range) -> visitExpr synExpr |> go
        ...
        | SynExpr.Record (tupleOption, copyInfo, synExprRecordFields, range) ->
            ...
            match copyInfo with
            | Some (synExpr, tuple) -> visitExpr synExpr |> go
            | None -> ()
        ...
        | SynExpr.Tuple (isStruct, synExprs, commaRanges, range) -> visitExprs synExprs |> go
        | SynExpr.Typar (synTypar, range) -> visitTypar synTypar |> go
        ...
        | SynExpr.While (debugPointAtWhile, whileExpr, synExpr, range) ->
            ...
            visitExpr synExpr |> go
        | SynExpr.AddressOf (isByref, synExpr, opRange, range) -> visitExpr synExpr |> go
        | SynExpr.AnonRecd (isStruct, tupleOption, recordFields, range) ->
            match tupleOption with
            | Some (synExpr, tuple) -> ()
            | None -> ()

            recordFields |> Seq.collect (fun (ident, range, f) -> visitExpr f) |> go
        | SynExpr.ComputationExpr (hasSeqBuilder, synExpr, range) -> visitExpr synExpr |> go
        | SynExpr.DebugPoint (debugPointAtLeafExpr, isControlFlow, innerExpr) -> visitExpr innerExpr |> go
        | SynExpr.DoBang (synExpr, range) -> visitExpr synExpr |> go
        ...
        | SynExpr.IndexRange (synExprOption, range, exprOption, range1, range2, range3) ->
            match synExprOption with
            | Some expr -> visitExpr expr |> go
            | None -> ()

            match exprOption with
            | Some expr -> visitExpr expr |> go
            | None -> ()
        | SynExpr.InferredDowncast (synExpr, range) -> visitExpr synExpr |> go
        | SynExpr.InferredUpcast (synExpr, range) -> visitExpr synExpr |> go
        ...
        | SynExpr.ObjExpr (objType, tupleOption, withKeyword, synBindings, synMemberDefns, synInterfaceImpls, newExprRange, range) ->
            ...
            match tupleOption with
            | Some (synExpr, identOption) -> visitExpr synExpr |> go
            | None -> ()
        ...
        | SynExpr.ArrayOrList (isArray, synExprs, range) -> visitExprs synExprs |> go
        ...
        | SynExpr.FromParseError (synExpr, range) -> visitExpr synExpr |> go
        | SynExpr.IfThenElse (synExpr, thenExpr, synExprOption, debugPointAtBinding, isFromErrorRecovery, range, synExprIfThenElseTrivia) ->
            ...
            | Some expr -> visitExpr expr |> go
            | None -> ()
        | SynExpr.IndexFromEnd (synExpr, range) -> visitExpr synExpr |> go
        ...
        | SynExpr.LongIdentSet (synLongIdent, synExpr, range) ->
            visitSynLongIdent synLongIdent |> go
            visitExpr synExpr |> go
        | SynExpr.YieldOrReturn (flags, synExpr, range) -> visitExpr synExpr |> go
        | SynExpr.ArrayOrListComputed (isArray, synExpr, range) -> visitExpr synExpr |> go
        ...
        | SynExpr.YieldOrReturnFrom (flags, synExpr, range) -> visitExpr synExpr |> go
        ...
        | SynExpr.DiscardAfterMissingQualificationAfterDot (synExpr, range) -> visitExpr synExpr |> go
    ...
    and visitStaticOptimizationConstraint (x: SynStaticOptimizationConstraint) : References =
        match x with
        | SynStaticOptimizationConstraint.WhenTyparIsStruct (synTypar, range) -> visitTypar synTypar
    ...
    and visitStaticOptimizationConstraints =
        visitMulti visitStaticOptimizationConstraint
    ...
    and visitSynAttributeList (attributeList: SynAttributeList) : References =
        attributeList.Attributes |> Seq.collect visitSynAttribute
    ...
    and visitSynModuleSigDecl (x: SynModuleSigDecl) : References =
        match x with
        | SynModuleSigDecl.Exception (synExceptionSig, range) -> visitSynExceptionSig synExceptionSig
        | SynModuleSigDecl.Open (synOpenDeclTarget, range) -> visitSynOpenDeclTarget synOpenDeclTarget
        | SynModuleSigDecl.Types (synTypeDefnSigs, range) -> visitSynTypeDefnSigs synTypeDefnSigs
        | SynModuleSigDecl.Val (synValSig, range) -> visitSynValSig synValSig
        ...
        | SynModuleSigDecl.NamespaceFragment synModuleOrNamespaceSig -> visitSynModuleOrNamespaceSig synModuleOrNamespaceSig
    ...
    and visitSynModuleOrNamespaceSig (x: SynModuleOrNamespaceSig) : References =
        match x with
        | SynModuleOrNamespaceSig.SynModuleOrNamespaceSig (longId,
                                                           isRecursive,
                                                           synModuleOrNamespaceKind,
                                                           synModuleDecls,
                                                           preXmlDoc,
    ...
    and findModuleAndTypeRefs (input: ParsedInput) =
        ...
        | ParsedInput.SigFile (ParsedSigFileInput (fileName,
            ...
            synModuleOrNamespaceSigs |> Seq.collect visitSynModuleOrNamespaceSig
        | ParsedInput.ImplFile (ParsedImplFileInput (fileName,
            ...
            synModuleOrNamespaces |> Seq.collect visitSynModuleOrNamespace
        |> Seq.distinct
        |> Seq.toArray
    ...
    let extractModuleSegments (stuff: ReferenceOrAbbreviation seq) : SimpleId[] * Abbreviation[] =
        let refs, abbreviations =
            ...
            |> Array.split (function
                | ReferenceOrAbbreviation.Reference r -> Choice1Of2 r
                | ReferenceOrAbbreviation.Abbreviation a -> Choice2Of2 a)
...
module TopModulesExtraction =
    ...
    let combine (parent: LongIdent) (children: Eit) =
        match children with
        | Eit.Nested idents -> idents |> Array.map (fun child -> List.append parent child)
        | Eit.SomeTypeLikeStuff -> [| parent |]
    ...
    and moduleDecl (x: SynModuleDecl) : Eit =
        ...
        | SynModuleDecl.ModuleAbbrev _ -> Eit.SomeTypeLikeStuff
        | SynModuleDecl.HashDirective _
        | SynModuleDecl.Open _ -> Eit.Nested [||] // Elements can be ignored
        | SynModuleDecl.NamespaceFragment synModuleOrNamespace -> topStuffForSynModuleOrNamespace synModuleOrNamespace |> Eit.Nested
        | SynModuleDecl.NestedModule (synComponentInfo, isRecursive, synModuleDecls, isContinuing, range, synModuleDeclNestedModuleTrivia) ->
    ...
    and moduleSigDecl (x: SynModuleSigDecl) : Eit =
        ...
        | SynModuleSigDecl.ModuleAbbrev _ -> Eit.SomeTypeLikeStuff
        | SynModuleSigDecl.HashDirective _
        | SynModuleSigDecl.Open _ -> Eit.Nested [||] // Elements can be ignored
        | SynModuleSigDecl.NamespaceFragment synModuleOrNamespace -> topStuffForSynModuleOrNamespaceSig synModuleOrNamespace |> Eit.Nested
        | SynModuleSigDecl.NestedModule (synComponentInfo, isRecursive, synModuleSigDecls, range, synModuleSigDeclNestedModuleTrivia) ->
            match synComponentInfo with
            | SynComponentInfo (synAttributeLists,
                                synTyparDeclsOption,
                ...
                let idents =
                    if mightHaveAutoOpen synAttributeLists then
                        ...
                        synModuleSigDecls |> moduleSigDecls |> combine longId
    ...
    let topModuleOrNamespaces (input: ParsedInput) =
        ...
        | ParsedInput.ImplFile f ->
            ...
            | items -> items |> List.toArray |> Array.collect topStuffForSynModuleOrNamespace
        | ParsedInput.SigFile f ->
            match f.Contents with
            | [] -> failwith $"No modules or namespaces found in file '{f.FileName}'"
            | items -> items |> List.toArray |> Array.collect topStuffForSynModuleOrNamespaceSig
