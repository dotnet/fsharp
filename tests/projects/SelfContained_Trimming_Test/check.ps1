...
{
    Write-Error "Test failed with unexpected FSharp.Core length:`nExpected:`n`t${expected_len} Bytes`nActual:`n`t${file_len} Bytes`nEiter codegen or trimming logic have changed. Please investigate and either update expected size or report an issue." -ErrorAction Stop
}