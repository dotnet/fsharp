...
"""
    ...
    let results = compileChecker.ParseAndCheckProject(options) |> Async.RunSynchronouslyImmediate
    ...
    for i in 1..3 do
        ...
        compileChecker.CompileFromCheckedProject(results, outFile) |> Async.RunSynchronouslyImmediate |> ignore
