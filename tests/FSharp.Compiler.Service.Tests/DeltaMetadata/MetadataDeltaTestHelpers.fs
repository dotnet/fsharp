...
module internal MetadataDeltaTestHelpers =
    ...
    /// (WriteILBinaryInMemoryWithArtifacts) that also returns token maps and a metadata
    /// snapshot; that variant belongs to a separate, larger baseline-capture change that is out
    /// of scope for this extraction, and every call site below only ever used the raw bytes.
    let createAssemblyBytes (moduleDef: ILModuleDef) =
        let options = defaultWriterOptions ilGlobals
        ILWriter.WriteILBinaryInMemory(options, moduleDef, id)

    /// Seed values for IlDeltaStreamBuilder read directly from a compiled baseline's bytes via
    /// SRM: (#US heap size, StandAloneSig row count). The feature branch derived these from the
    /// hot-reload baseline module's MetadataSnapshot type (out of scope here); reading them off
    /// the baseline's own metadata is equivalent for these tests and keeps this helper file free
    /// of hot-reload imports.
    let private builderSeed (bytes: byte[]) =
        use peReader = new PEReader(new MemoryStream(bytes, false))
        let metadataReader = peReader.GetMetadataReader()
        metadataReader.GetHeapSize HeapIndex.UserString, metadataReader.GetTableRowCount TableIndex.StandAloneSig
    ...
    let private emitPropertyDeltaCore
        ...
        let methodRow: DeltaWriter.MethodDefinitionRowInfo =
            { Key = methodKey
              RowId = 1
              IsAdded = true
              ParentTypeDefRowId = Some(MetadataTokens.GetRowNumber typeHandle)
        ...
        let propertyRows: DeltaWriter.PropertyDefinitionRowInfo list =
            [ { Key = propertyKey
                RowId = 1
                IsAdded = true
                // Resolved by the writer from the PropertyMap rows below.
    ...
    let private emitPropertyDeltaFromBaseline (baselineBytes: byte[]) (heapOffsets: MetadataHeapOffsets) (generation: int) (encBaseId: Guid) =
        ...
        let metadataReader = peReader.GetMetadataReader()
        let userStringHeapSize, standAloneSigRowCount = builderSeed baselineBytes
        let builder = IlDeltaStreamBuilder(userStringHeapSize, standAloneSigRowCount)
    ...
    let emitPropertyDeltaArtifacts (messageLiteral: string option) () : MetadataDeltaArtifacts =
        ...
        let assemblyBytes, _ = createAssemblyBytes moduleDef
    ...
    let private emitLocalSignatureDeltaCore
        ...
        let methodRow: DeltaWriter.MethodDefinitionRowInfo =
            { Key = methodKey
              RowId = 1
              IsAdded = true
              ParentTypeDefRowId = Some(MetadataTokens.GetRowNumber(methodDef.GetDeclaringType()))
    ...
    let emitLocalSignatureDeltaArtifacts (messageLiteral: string option) () : MetadataDeltaArtifacts =
        ...
        let metadataReader = peReader.GetMetadataReader()
        let baselineHeapSizes = getHeapSizes metadataReader
        // Seed from the real baseline: this helper copies a NON-NIL baseline local signature
        // into a new StandAloneSig row, so the row id must continue from the baseline row
        // count (baseline + 1, Roslyn parity), not restart at 1.
        let userStringHeapSize, standAloneSigRowCount = builderSeed assemblyBytes
        let builder = IlDeltaStreamBuilder(userStringHeapSize, standAloneSigRowCount)
        let heapOffsets = computeHeapOffsets metadataReader
    ...
    let private emitEventDeltaCore
        ...
        let methodRow : DeltaWriter.MethodDefinitionRowInfo =
            { Key = methodKey
              RowId = 1
              IsAdded = true
              ParentTypeDefRowId = Some(MetadataTokens.GetRowNumber(addDef.GetDeclaringType()))
        ...
        let eventRows: DeltaWriter.EventDefinitionRowInfo list =
            [ { Key = eventKey
                RowId = 1
                IsAdded = true
                // Resolved by the writer from the EventMap rows below.
