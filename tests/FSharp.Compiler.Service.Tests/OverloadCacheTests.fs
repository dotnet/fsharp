...
[<Fact>]
let ``CreateOverloadCacheMetricsListener returns valid listener`` () =
    // Test the new public API on FSharpChecker
    use listener = FSharpChecker.CreateOverloadCacheMetricsListener()
    
    // Generate source with overload calls
    let callCount = 50
    let source = generateRepetitiveOverloadCalls callCount
    ...
    // Verify listener captured some metrics
    let totals = listener.GetTotals()
    let hits = listener.Hits
    let misses = listener.Misses
    let ratio = listener.Ratio
    
    // We should have some cache activity
    printfn "Overload cache metrics via CreateOverloadCacheMetricsListener:"
    printfn "  Hits: %d, Misses: %d, Ratio: %.2f%%" hits misses (ratio * 100.0)
    printfn "  Totals: %A" totals
    
    // With 50 repetitive calls, we should have some activity (either hits or misses)
    Assert.True(hits + misses > 0L, "Cache should have some activity")
