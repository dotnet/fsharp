...
let csharpValues1 = (NormalizationFormKCCSharp_0000.Values()).GetEnumerator()
let csharpValues2 = (NormalizationFormKCCSharp_0080.Values()).GetEnumerator()
let fsharpValues1 = (NormalizationFormKCFSharp_0000.Values()).GetEnumerator()
let fsharpValues2 = (NormalizationFormKCFSharp_0080.Values()).GetEnumerator()
...
for i in [0 .. 0x07F] do
    ...
    if csv1 <> fsv1 then
        raise (new Exception($"Difference detected NormalizationFormatKC at: '{i}': '{csv1} <> '{fsv1}'"))

    if csv2 <> fsv2 then
        raise (new Exception($"Difference detected NormalizationFormatKC at: '{i}': '{csv2} <> '{fsv2}'"))
()
