...
let fsharpValues1 = (NormalizationFormKDFSharp_0000.Values()).GetEnumerator()
let fsharpValues2 = (NormalizationFormKDFSharp_0080.Values()).GetEnumerator()
...
for i in [0 .. 0x07F] do
    ...
    if csv1 <> fsv1 then
        raise (new Exception($"Difference detected NormalizationFormatKD at: '{i}': '{csv1} <> '{fsv1}'"))

    if csv2 <> fsv2 then
        raise (new Exception($"Difference detected NormalizationFormatKD at: '{i}': '{csv2} <> '{fsv2}'"))
()
