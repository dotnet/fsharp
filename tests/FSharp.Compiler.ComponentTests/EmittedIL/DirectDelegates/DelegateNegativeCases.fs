...
// 44. first-class function value: there is no target method to point at, so a closure must remain
let firstClass (handler: int -> int -> unit) = Action<int, int>(handler)

// 45. lambda body is not a single direct forwarding call to a known target (the argument is computed,
...
// 46. arguments reordered: not a transparent forwarding, so a closure must remain
...
// 47. Reference-parameter contravariance: the delegate's Invoke is (string):int and the target is (object):int.
