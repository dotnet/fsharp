...
module times =
    ...
    [<Theory; Directory(__SOURCE_DIRECTORY__ + "/../../resources/tests/CompilerOptions/fsc/times", Includes=[|"times01.fs"|])>]
    let ``times - times01.fs - --times`` compilation =
        compilation
        |> withOptions ["--times"]
        |> typecheck
        |> shouldSucceed
        |> withDiagnosticMessageMatches "TIME:.+Delta:.+Mem:.+G0:.+G1:.+G2:.+\[Write .NET Binary\]"

    // This test was automatically generated (moved from FSharpQA suite - CompilerOptions/fsc/times)
    //<Expects status="success">TIME:.+Delta:.+Mem:.+G0:.+G1:.+G2:.+\[Write .NET Binary\]</Expects>
    [<Theory; Directory(__SOURCE_DIRECTORY__ + "/../../resources/tests/CompilerOptions/fsc/times", Includes=[|"times01.fs"|])>]
    let ``times - times01.fs - --times --times`` compilation =
        compilation
        |> withOptions ["--times"; "--times"]
        |> typecheck
        |> shouldSucceed
        |> withDiagnosticMessageMatches "TIME:.+Delta:.+Mem:.+G0:.+G1:.+G2:.+\[Write .NET Binary\]"
