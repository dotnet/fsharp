...
module GenericAttributeAbbreviations =
    ...
    // FS0193 "The lists had different lengths" - it must report FS3888.
    [<Fact>]
    let ``Type abbreviation of generic attribute reports FS3888 instead of crashing`` () =
        Fsx """
...
"""
    ...
    let ``Generic attribute abbreviation variants all report FS3888`` (abbrev: string) =
        Fsx (sprintf """
type A<'T>() = inherit System.Attribute()
%s
[<B>] type C = class end
""" abbrev)
    ...
    [<Fact>]
    let ``Two-parameter generic attribute abbreviation reports FS3888`` () =
        Fsx """
...
"""
    ...
    [<Fact>]
    let ``Chained abbreviation through a generic attribute reports FS3888`` () =
        Fsx """
type A<'T>() = inherit System.Attribute()
type B = A<int>
type C2 = B
