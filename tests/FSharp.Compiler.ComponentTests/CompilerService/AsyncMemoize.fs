...
let ``Preserve thread static diagnostics`` () = 
    ...
    let diagnosticMessages = results |> Seq.map snd |> Seq.map (Array.map _.PhasedDiagnostic.Exception.Message >> Array.toList) |> Set
...
let ``Preserve thread static diagnostics already completed job`` () =
    ...
    task {
        ...
        let diagnosticMessages = diagnosticsLogger.GetDiagnostics() |> Array.map _.PhasedDiagnostic.Exception.Message |> Array.toList
...
let ``We get diagnostics from the job that failed`` () =
    ...
    task {
        ...
        let messages = logger.Diagnostics |> List.map _.PhasedDiagnostic.Exception.Message
