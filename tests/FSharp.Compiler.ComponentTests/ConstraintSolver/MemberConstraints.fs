...
"""
    ...
    [<InlineData("8.0","9.0")>]
    [<Theory>]
    let ``FsharpPlus style SRTP backwards compat broken`` (langVersionApp, langVersionLibrary) =

        let fsharpPlusLib =
            FsFromPath (Path.Combine(__SOURCE_DIRECTORY__, "FsharpPlusLikeForNet8.fs"))
            |> withName "fsharpPlusLib.fs"
            |> withLangVersion langVersionLibrary
            |> asLibrary

        FsFromPath (Path.Combine(__SOURCE_DIRECTORY__, "ConsumerOfFSharpPlusNetCurrent.fs"))
        |> withName "appConsumer.fs"
        |> withReferences [ fsharpPlusLib ]
        |> asExe
        |> withLangVersion langVersionApp
        |> compile
        |> shouldSucceed
