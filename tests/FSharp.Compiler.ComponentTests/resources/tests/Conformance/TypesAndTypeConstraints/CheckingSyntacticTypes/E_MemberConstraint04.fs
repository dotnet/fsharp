...
//<Expects status="error" span="(18,28-18,39)" id="FS0193">Type constraint mismatch\. The type.+'OtherFoo'.+is not compatible with type.+'Foo'</Expects>
//<Expects status="error" span="(19,20-19,31)" id="FS0193">This expression was expected to have type.+'OtherFoo'.+but here has type.+'Foo'</Expects>
//<Expects status="error" span="(19,34-19,40)" id="FS0193">Type constraint mismatch\. The type.+'Foo'.+is not compatible with type.+'OtherFoo'</Expects>
