...
[<Fact>]
let ``Copy-and-update field should not be classified as type name`` () : Task =
    task {
        let source =
            """
module Test
...
let expectUnionCaseClassifications source (expectations: (int * int * int) list) : Task =
    task {
        let! items = getClassifications source
...
[<Fact>]
let ``Union case tester classification range should not include dot`` () : Task =
    let source =
        """
module Test
...
[<Fact>]
let ``Union case tester classification across scenarios`` () : Task =
    let source =
        """
module Test
...
[<Fact>]
let ``Static method on generic type should classify type name as type`` () : Task =
    task {
        let source =
            """
module Test
...
[<Fact>]
let ``Undeclared identifier in expression position is not classified as a type`` () : Task =
    task {
        let source =
            """
module Test
...
[<Fact>]
let ``Delegate Invoke in declaration not classified as method`` () : Task =
    task {
        let source = """
...
[<Fact>]
let ``Delegate Invoke at call site classified as method`` () : Task =
    task {
        let source = """
type MyDelegate = delegate of int -> string
...
[<Fact>]
let ``Generic delegate Invoke not classified as method in decl`` () : Task =
    task {
        let source = """
...
[<Fact>]
let ``BeginInvoke EndInvoke also not classified in decl`` () : Task =
    task {
        let source = """
type MyDelegate = delegate of int -> string
"""
...
[<Fact>]
let ``IDisposable in interface impl classified as interface`` () : Task =
    task {
        let source = """
open System
type MyClass() =
...
[<Fact>]
let ``Concrete disposable class classified as disposable type`` () : Task =
    task {
        let source = """
open System.IO
...
[<Fact>]
let ``IDisposable as type constraint classified as interface`` () : Task =
    task {
        let source = """
open System
...
[<Fact>]
let ``Non-IDisposable interface classified as interface`` () : Task =
    task {
        let source = """
type IMyInterface =
...
[<Fact>]
let ``Issue 19905 item 1 - tupled delegate decl has no Method classification`` () : Task =
    task {
        let source = """
...
let ``Issue 19905 item 6 - MailboxProcessor int dot Start classified correctly`` () : Task =
    task {
        let source = """
...
[<Fact>]
let ``Issue 19905 item 2 - CE inside list comprehension classified as ComputationExpression`` () : Task =
    task {
        let source = """
...
[<Fact>]
let ``Issue 19905 item 5 - open-ended slice closing bracket not classified as Function`` () : Task =
    task {
        let source = "\nlet list = [1; 2; 3]\nlet x    = list[0..]\n"
        let! classifications = getClassifications source
        let lines = source.Replace("\r\n", "\n").Split('\n')
...
[<Fact>]
let ``Issue 19905 item 7 - open type not reported as unused`` () : Task =
    task {
...
"""
        ...
        let! fileName, snapshot, checker = singleFileChecker source
        let! parseResults, checkAnswer = checker.ParseAndCheckFileInProject(fileName, snapshot)
        ...
        let! unused = UnusedOpens.getUnusedOpens(checkResults, getSourceLineStr)
...
[<Fact>]
let ``Issue 19905 item 7 - open type used via static field not reported as unused`` () : Task =
    task {
        let source = """module Test
...
[<Fact>]
let ``Issue 19905 item 5 - open-ended array slice closing bracket not classified`` () : Task =
    task {
        let source = "\nlet arr = [|1;2;3|]\nlet x = arr[0..]\n"
        let! items = getClassifications source
...
[<Fact>]
let ``Issue 19905 item 7 - open type unused when only an instance record field is used`` () : Task =
    task {
        let source = """module Test
type R = { X: int }
open type R
...
[<Fact>]
let ``Issue 19905 item 7 - open type used only via fully-qualified static member is consistent with module opens`` () : Task =
    task {
        let source =
            """module Test
module M =
...
let private assertIdentNeverMutable (source: string) (ident: string) : Task =
    task {
        let! items = classificationsForIdent source ident
        let mutableHits = items |> Array.filter (fun item -> item.Type = SemanticClassificationType.MutableVar)
        Assert.True(
...
[<Fact>]
let ``5229 class as this is not MutableVar`` () : Task =
    let source = """
module Test
type C() as this =
    member _.F() = this
...
type C() as this =
    ...
    member _.C() = this
""", "this")>]
let ``5229 as this variants are not MutableVar`` (_label: string) (source: string) (ident: string) : Task =
...
let () = r := !r + 1
""", "r")>]
let ``5229 real mutable bindings remain MutableVar`` (_label: string) (source: string) (ident: string) : Task =
    task {
        let! items = classificationsForIdent source ident
        let mutableHits =
...
[<Fact>]
let ``5229 mutable record field updates remain MutableRecordField`` () : Task =
    task {
        let source = """
module Test
