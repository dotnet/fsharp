...
"""
        checkAllSymbols source (fun _ allUses ->
            ...
            let ctorSymbols =
                ...
                |> Seq.toArray
            // All constructors must be at column 2 (the correct range).
            // The regression had a constructor at column 3 (shifted by +1).
            for (startCol, endCol) in ctorSymbols do
                Assert.Equal(2, startCol)
                Assert.Equal(8, endCol)
            // At least one constructor must exist.
            Assert.True(ctorSymbols.Length >= 1, sprintf "Expected at least 1 constructor, got %d" ctorSymbols.Length))
