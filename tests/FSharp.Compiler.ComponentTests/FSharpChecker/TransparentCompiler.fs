...
let GiraffeTest () =
    ...
    let project = SyntheticProject.CreateFromRealProject (__SOURCE_DIRECTORY__ ++ ".." ++ ".." ++ ".." ++ ".." ++ "Giraffe-signatures/src/Giraffe")
    let project = { project with OtherOptions = "--nowarn:FS3520"::project.OtherOptions }
    ...
    let modify (sourceFile: SyntheticSourceFile) =
        { sourceFile with
            Source = addComment sourceFile.Source
            SignatureFile =
                match sourceFile.SignatureFile with
                | Custom signature -> Custom (addComment signature)
                | x -> x
            }

    let break' (sourceFile: SyntheticSourceFile) =
        { sourceFile with
            Source = prependSlash sourceFile.Source
            SignatureFile =
                match sourceFile.SignatureFile with
                | Custom signature -> Custom (prependSlash signature)
                | x -> x
            }
