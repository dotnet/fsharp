...
let (|OddVOption|_|) x = if x % 2 = 1 then ValueSome() else ValueNone
    ...
    |> shouldFail
    |> withDiagnostics [
        (Error 3350, Line 1, Col 6, Line 1, Col 17, "Feature 'Boolean-returning and return-type-directed partial active patterns' is not available in F# 8.0. Please use language version 10.0 or greater.")
        (Error 3350, Line 2, Col 6, Line 2, Col 20, "Feature 'Boolean-returning and return-type-directed partial active patterns' is not available in F# 8.0. Please use language version 10.0 or greater.")
    ]
