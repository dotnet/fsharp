...
module Rethrowing =
    ...
    [<Fact>]
    let ``reraise in a finally inside the handler`` () =
        assertingRethrow "" """
        async {
            try
                throwIt ()
...
module HandlerPattern =
    /// What is rethrown is what the handler caught, not what the pattern bound, so a pattern that never names
    /// the exception rethrows it all the same.
    [<Fact>]
    let ``a pattern that does not name the exception still rethrows it`` () =
        preamble
        + """
let thrown =
    try
...
module HandlerType =
    /// The builder leaves what its handler catches to inference, and the clause body solves it to 'exn' only
    /// after the 'reraise ()' has been checked.
    [<Fact>]
    let ``solved to exn after the reraise is checked`` () =
        assertingRethrow
            """
type Builder() =
    ...
    member _.TryWith(body: unit -> 'T, handler: 'E -> 'T) =
        try body () with e -> handler (unbox<'E> (box e))
...
module SequenceExpressionGuards =
    /// A guard runs in the filter pass, where the rethrow abandons the filter, so no clause is selected and the
    /// exception carries on with the guard's frame and a dispatch boundary added.
    [<Fact>]
    let ``a guard that rethrows runs once and skips the remaining clauses`` () =
        preamble
        + """
let guardRuns = ref 0
    ...
    try
        seq {
            try
                ...
                yield 1
            with
            | _ when (guardRuns.Value <- guardRuns.Value + 1; reraise ()) -> yield 2
            | _ -> yield 3
        }
        |> Seq.toList
        |> ignore
...
[<EntryPoint>]
let main _ =
    if guardRuns.Value <> 1 then failwith $"expected the guard to run once, ran {guardRuns.Value} times"
    match thrown with
    | Some (Original "boom") -> 0
    | other -> failwith $"expected the original exception, got {other}"
"""
...
"""
    ...
    /// Only the 'with' clause of the nested try rebinds it; its protected body is still part of the computation
    /// expression handler, so a 'reraise ()' there rethrows the exception the computation expression caught.
    [<Fact>]
    let ``reraise in the protected body rethrows the outer exception`` () =
        preamble
        + """
[<EntryPoint>]
let main _ =
    async {
        ...
        with e ->
            ...
            try
                return reraise ()
            with inner ->
                if not (Object.ReferenceEquals(inner, handled.Value)) then
                    failwith $"expected the exception the computation expression caught, got {inner}"
                return 0
    }
    |> Async.RunSynchronously
"""
...
module Quotations =
    /// A quotation carries the rewritten form, so anything evaluating it sees the dispatch calls rather than a
    /// 'reraise' it has no way to honour.
    [<Fact>]
    let ``a quoted handler carries the ExceptionDispatchInfo rethrow`` () =
        """
module Test
...
let rec calledMethods expr =
    seq {
        match expr with
        | Patterns.Call(_, mi, _) -> yield mi.Name
        | _ -> ()

        match expr with
        | ShapeVar _ -> ()
        | ShapeLambda(_, body) -> yield! calledMethods body
        | ShapeCombination(_, args) -> for arg in args do yield! calledMethods arg
    }

[<EntryPoint>]
let main _ =
    let called = calledMethods q |> Set.ofSeq
    if not (called.Contains "Capture" && called.Contains "Throw") then
        failwith $"expected an ExceptionDispatchInfo rethrow, called {called}"
    0
"""
        |> FSharp
        |> asExe
