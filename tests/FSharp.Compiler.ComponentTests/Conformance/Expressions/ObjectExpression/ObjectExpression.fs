...
let foo = { new Foo() with member __.ToString() = base.ToString() }
    ...
    [<Fact>]
    let ``Object expression can not implementin an interface when it contains a method with no types that can refer to the type for which the implementation is being used`` () =
        Fsx """
type AsString =
    abstract member asString: unit -> string
