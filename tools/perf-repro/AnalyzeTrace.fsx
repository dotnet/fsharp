...
let generateReport config =
    ...
    // Header
    sb.AppendLine("# F# Compiler Performance Analysis - xUnit Assert.Equal Issue #18807")
    |> ignore

    sb.AppendLine() |> ignore

    sb.AppendLine(sprintf "*Generated: %s*" (DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss")))
    |> ignore
    ...
    if File.Exists(summaryPath) then
        ...
        sb.AppendLine(sprintf "- **Total Assert.Equal calls**: %s" totalAsserts)
        |> ignore

        sb.AppendLine(sprintf "- **Test methods**: %s" testMethods) |> ignore

        sb.AppendLine("- **Type variants**: int, string, float, bool, int64, decimal, byte, char")
        |> ignore

        sb.AppendLine() |> ignore

    // Parse timing files
    let untypedTimingPath =
        Path.Combine(config.ResultsDir, "XUnitPerfTest.Untyped.timing.txt")

    let typedTimingPath =
        Path.Combine(config.ResultsDir, "XUnitPerfTest.Typed.timing.txt")
    ...
    | Some(time, perAssert) ->
        ...
        sb.AppendLine(sprintf "- **Total compilation time**: %.2f seconds" time)
        |> ignore

        sb.AppendLine(sprintf "- **Time per Assert.Equal**: %.2f ms" perAssert)
        |> ignore
    ...
    let untypedTracePath =
        Path.Combine(config.ResultsDir, "XUnitPerfTest.Untyped.nettrace")
    ...
    if File.Exists(untypedTracePath) || File.Exists(typedTracePath) then
        ...
        | Some analysis ->
            ...
            sb.AppendLine() |> ignore
        | None ->
            sb.AppendLine("*Note: Detailed trace analysis not available. Install dotnet-trace for detailed profiling.*")
            |> ignore

            sb.AppendLine() |> ignore
    else
        sb.AppendLine("*Note: No trace files found. Trace collection may have failed or been skipped.*")
        |> ignore

        sb.AppendLine("*For detailed profiling, ensure dotnet-trace is installed and has proper permissions.*")
        |> ignore
    ...
    | (Some(untypedTime, untypedPerAssert), Some(typedTime, typedPerAssert)) ->
        if untypedPerAssert > 10.0 then
            sb.AppendLine(
                sprintf
                    "⚠️ **Critical**: Each untyped `Assert.Equal` call adds approximately **%.2f ms** to compilation time."
                    untypedPerAssert
            )
            |> ignore

            sb.AppendLine(sprintf "In contrast, typed calls add only **%.2f ms** each." typedPerAssert)
            |> ignore
        ...
        if untypedTime / typedTime > 3.0 then
            sb.AppendLine(
                sprintf
                    "⚠️ **Severe Slowdown**: The untyped version is **%.1fx slower** than the typed version."
    ...
    sb.AppendLine("Based on the issue discussion and F# compiler architecture:")
    |> ignore
    ...
    sb.AppendLine("   - F# compiler tries each overload during type inference")
    |> ignore

    sb.AppendLine("   - Each attempt typechecks the full overload signature")
    |> ignore

    sb.AppendLine("   - Location: `src/Compiler/Checking/ConstraintSolver.fs` around line 3486")
    |> ignore

    sb.AppendLine() |> ignore
    sb.AppendLine("2. **Type Inference Without Explicit Types**") |> ignore

    sb.AppendLine("   - Untyped calls force the compiler to infer types from usage")
    |> ignore

    sb.AppendLine("   - This requires constraint solving for each Assert.Equal call")
    |> ignore
    ...
    sb.AppendLine("   - Each Assert.Equal call repeats the same expensive analysis")
    |> ignore
    ...
    sb.AppendLine("- **Location**: `src/Compiler/Checking/ConstraintSolver.fs`")
    |> ignore

    sb.AppendLine("- **Opportunity**: Cache overload resolution results for identical call patterns")
    |> ignore

    sb.AppendLine("- **Expected Impact**: Could reduce compilation time by 50-80% for repetitive patterns")
    |> ignore

    sb.AppendLine("- **Rationale**: Many Assert.Equal calls have identical type patterns")
    |> ignore
    ...
    sb.AppendLine("- **Location**: `src/Compiler/Checking/MethodCalls.fs`")
    |> ignore

    sb.AppendLine("- **Opportunity**: Filter incompatible overloads before full type checking")
    |> ignore

    sb.AppendLine("- **Expected Impact**: Could reduce time by 30-50%") |> ignore

    sb.AppendLine("- **Rationale**: Many overloads can be ruled out based on argument count/types")
    |> ignore
    ...
    sb.AppendLine("- **Location**: `src/Compiler/Checking/TypeChecker.fs`")
    |> ignore

    sb.AppendLine("- **Opportunity**: Reuse partial type information across similar calls")
    |> ignore
    ...
    sb.AppendLine("   Assert.Equal<int>(expected, actual)  // Explicit type")
    |> ignore
    ...
    sb.AppendLine("   let assertEqual (x: 'T) (y: 'T) = Assert.Equal<'T>(x, y)")
    |> ignore
    ...
    sb.AppendLine("1. **Profile with Real Traces**: Use PerfView or dotnet-trace to identify exact bottlenecks")
    |> ignore

    sb.AppendLine("2. **Focus on ConstraintSolver.fs**: This is the likely hot path")
    |> ignore

    sb.AppendLine("3. **Consider Overload Resolution Cache**: Biggest potential impact")
    |> ignore

    sb.AppendLine("4. **Benchmark Improvements**: Use this test suite to validate optimizations")
    |> ignore
    ...
    sb.AppendLine("| Metric | Untyped (Slow) | Typed (Fast) | Difference |")
    |> ignore
    ...
    | (Some(ut, upa), Some(tt, tpa)) ->
        sb.AppendLine(sprintf "| Total Time | %.2fs | %.2fs | %.2fs |" ut tt (ut - tt))
        |> ignore

        sb.AppendLine(sprintf "| Time/Assert | %.2fms | %.2fms | %.2fms |" upa tpa (upa - tpa))
        |> ignore

        sb.AppendLine(sprintf "| Slowdown | %.2fx | 1.0x | - |" (ut / tt)) |> ignore
    | _ -> sb.AppendLine("| N/A | N/A | N/A | N/A |") |> ignore
    ...
    sb.AppendLine("*This report was automatically generated by the F# compiler performance profiling suite.*")
    |> ignore

    sb.AppendLine("*For more information, see issue [#18807](https://github.com/dotnet/fsharp/issues/18807).*")
    |> ignore
