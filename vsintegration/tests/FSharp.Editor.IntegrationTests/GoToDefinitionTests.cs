...
{
    // What the CI failures taught us about why a naive GoToDefn was a no-op -- and why no retry loop is
    // needed once those are addressed:
    //
    //   * Resolution is SYNCHRONOUS. FSharpNavigation.TryGoToDefinition blocks on the checker
    //     (gtdTask.Wait()), so once the project is loaded the symbol is resolved on the very first
    //     invocation -- re-issuing the command never helped. GoesToDefinition's no-op was simply that an
    //     un-built project has no checker options, so resolution returned nothing; building the project
    //     (see the test body) fixes that.
    //   * Navigation is ASYNCHRONOUS. FSharpNavigation.NavigateToItem schedules the actual caret move as UI
    //     work via JoinableTaskFactory, so the caret has NOT moved yet by the time
    //     Shell.ExecuteCommandAsync(GotoDefn) returns. Sampling the caret immediately (or re-anchoring it,
    //     as an earlier retry loop did) races that navigation and makes the caret visibly bounce between the
    //     call site and the definition.
    //
    // So we issue GoToDefn once and then wait for that single navigation to land.
    ...
    {
        // Make sure the project is loaded into the workspace, the editor is the active command context
        // (PlaceCaretAsync's dte.Find moves VS's active selection off the editor), and the caret is on the
        // call site.
        ...
        // Wait for the single asynchronous navigation to move the caret off the call site.
