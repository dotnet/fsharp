...
let arr = [| 1; 2; 3 |]
            """,
            expectedMessage = "Incompatible arguments. Are you accidentally passing in too many arguments to a function, or arguments to a simple value? Alternatively, you might need to supply parentheses to tell the compiler the order of execution e.g. 'add 1 add 2 3' might need to be rewritten as 'add 1 (add 2 3)'.")
