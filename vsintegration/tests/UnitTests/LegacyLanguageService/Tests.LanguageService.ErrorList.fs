...
type UsingMSBuild() as this = 
    ...
    let GetWarnings(project : OpenProject) =
        ...
        [for error in errorList do
            if (error.Severity = Microsoft.VisualStudio.LegacyLanguageService.Severity.Warning) then
            yield error]
    ...
    member public this.``TypeProvider.MultipleErrors`` () =
        ...
        let checkList n = 
            ...
            for x = 0 to (n - 1) do
                ...
                Assert.IsTrue(Set.contains (expectedName, Microsoft.VisualStudio.LegacyLanguageService.Severity.Error) uniqueErrors)
