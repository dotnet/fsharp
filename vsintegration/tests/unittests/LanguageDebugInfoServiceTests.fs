...
let main argv = 
    ...
    member this.TestDebugInfo(searchToken: string, expectedDataTip: string option) = 
        ...
        let tokens = FSharpColorizationService.GetColorizationData(documentId, sourceText, TextSpan.FromBounds(0, sourceText.Length), Some(fileName), defines, CancellationToken.None)
