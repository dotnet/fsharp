...
type internal FSharpProjectNode(package:FSharpProjectPackage) as this = 
    ...
    override this.EnsureMSBuildAndSolutionExplorerAreInSync() =
        let solnExplorer =
            this.AllDescendants
            |> Seq.choose (function
                | :? FSharpFileNode as node when node.ItemNode.Item.ItemType = ProjectFileConstants.Compile ->
                    Some node.Caption
                | _ ->
                    None
            )
    ...
    member this.SyncWithHierarchy(movement: SourceMovement.Movement) =
        ...
        | None -> ()
        | Some (oldSources, flags) ->
            let newCompileItems =
                this.BuildProject.AllEvaluatedItems
                |> Seq.filter (fun i -> i.ItemType = ProjectFileConstants.Compile)
                |> Seq.map (fun i -> System.IO.Path.Combine(this.ProjectFolder, i.EvaluatedInclude))
                |> Array.ofSeq
            ...
            | None ->
                // We failed to determine the new compile items
                // Be cautious and use the old, slow (but accurate) method instead
                this.ComputeSourcesAndFlags()
...
type internal FSharpFileNode(root:FSharpProjectNode, e:ProjectElement, hierarchyId) = 
    ...
    static member MoveDown(node : HierarchyNode, root : FSharpProjectNode) =
        ...
        root.SyncWithHierarchy(SourceMovement.Reorder)
