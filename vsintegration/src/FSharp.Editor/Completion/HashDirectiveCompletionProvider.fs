...
type internal HashDirectiveCompletionProvider
    ...
    let includeDirectiveCleanRegex =
        Regex("""#I\s+(@?"*(?<literal>[^"]*)"?)""", RegexOptions.Compiled ||| RegexOptions.ExplicitCapture)
    ...
    let getIncludeDirectives (text: SourceText, position: int) =
        ...
        |> Seq.choose (fun line ->
            ...
            if not (lineStr.StartsWith "#I") then
                None
            else
                ...
                | m when m.Success ->
                    ...
                    |> Seq.tryPick (fun span ->
                        if
                            span.TextSpan.IntersectsWith line.Start
                            && (span.ClassificationType <> ClassificationTypeNames.Comment
                                && span.ClassificationType <> ClassificationTypeNames.ExcludedCode)
                        then
        ...
        asyncMaybe {
            ...
            let helper =
                FSharpFileSystemCompletionHelper(
                    Glyph.OpenFolder,
                    completion.AllowableExtensions
                    |> List.tryPick getFileGlyph
                    |> Option.defaultValue Glyph.None,
    ...
    override _.IsInsertionTrigger(text, position) =
        ...
        let isTriggerChar =
            ...
            || FSharpCommonCompletionUtilities.IsStartingNewWord(
                text,
                position,
                (fun x -> Char.IsLetter x),
                (fun x -> Char.IsLetterOrDigit x)
            )
