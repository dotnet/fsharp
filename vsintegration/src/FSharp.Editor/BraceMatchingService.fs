...
type internal FSharpBraceMatchingService 
    ...
    member this.GetBraceMatchingResult(sourceText, fileName, options, position: int) = async {
        let! matchedBraces = checkerProvider.Checker.MatchBracesAlternate(fileName, sourceText.ToString(), options)
        ...
        member this.FindBracesAsync(document, position, cancellationToken) = 
            async {
                ...
                | Some options ->
                    ...
                    let! result = this.GetBraceMatchingResult(sourceText, document.Name, options, position)
