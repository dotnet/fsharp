...
type internal FSharpEditorFormattingService [<ImportingConstructor>] (settings: EditorOptions) =
    ...
    static let getIndentation (line: string) =
        ...
        asyncMaybe {
            ...
            if right.StartColumn = firstMeaningfulToken.LeftColumn then
                ...
                let indentChars (line: TextLine) =
                    line.ToString() |> Seq.takeWhile ((=) ' ') |> Seq.length

                let startIndent =
                    indentChars sourceText.Lines.[sourceText.Lines.IndexOf leftSpan.Start]
    ...
    static member GetPasteChanges
        (
            documentId: DocumentId,
            sourceText: SourceText,
            filePath: string,
            formattingOptions: Microsoft.VisualStudio.FSharp.Editor.FormattingOptions,
            tabSize: int,
            parsingOptions: FSharpParsingOptions,
        ...
        asyncMaybe {
            ...
            let fixedSpan =
                ...
                if currentClipboard.Length > 0 && pasteTextString.EndsWith currentClipboard then
                    let prepended =
                        pasteTextString.[0 .. pasteTextString.Length - currentClipboard.Length - 1]
            ...
            let currentIndent =
                let priorStartSpan =
                    TextSpan(startLine.Span.Start, startLine.Span.Length - (startLine.Span.End - fixedSpan.Start))
            ...
            if leadingIndentation > 0 then
                ...
                let nextLineShouldBeIndented =
                    FSharpIndentationService.IndentShouldFollow(documentId, sourceText, filePath, span.Start, parsingOptions)
    ...
    interface IFSharpEditorFormattingServiceWithOptions with
        ...
        override _.SupportsFormattingOnTypedCharacter(_document, _ch) = false
