...
type internal FSharpDocumentDiagnosticAnalyzer() =
    ...
    static member GetDiagnostics(checker: FSharpChecker, filePath: string, sourceText: SourceText, textVersionHash: int, parsingOptions: FSharpParsingOptions, options: FSharpProjectOptions, diagnosticType: DiagnosticsType) = 
        async {
            let! parseResults = checker.ParseFile(filePath, Source.SourceText(sourceText.ToFSharpSourceText()), parsingOptions, userOpName=userOpName) 
            let! errors = 
                async {
                    match diagnosticType with
                    | DiagnosticsType.Semantic ->
                        let! checkResultsAnswer = checker.CheckFileInProject(parseResults, filePath, textVersionHash, Source.SourceText(sourceText.ToFSharpSourceText()), options, userOpName=userOpName) 
