...
type internal GoToDefinition(checker: FSharpChecker, projectInfoManager: FSharpProjectOptionsManager) =
    ...
    member __.UserOpName = "GoToDefinition"
    ...
    member private this.FindSymbolHelper (originDocument: Document, originRange: range, sourceText: SourceText, preferSignature: bool) : Async<FSharpNavigableItem option> =
        asyncMaybe {
            ...
            let! _, _, checkFileResults = checker.ParseAndCheckDocument (originDocument, projectOptions, allowStaleResults=true,sourceText=sourceText, userOpName=this.UserOpName)
            let idRange = lexerSymbol.Ident.idRange
            let! fsSymbolUse = checkFileResults.GetSymbolUseAtLocation (fcsTextLineNumber, idRange.EndColumn, lineText, lexerSymbol.FullIsland, userOpName=this.UserOpName)
            ...
            if isSignatureFile fsSymbolUse.FileName && preferSignature = false then 
                ...
                let! _, _, checkFileResults = checker.ParseAndCheckDocument (implDoc, projectOptions, allowStaleResults=true, sourceText=implSourceText, userOpName=this.UserOpName)
    ...
    member this.FindDeclarationOfSymbolAtRange(targetDocument: Document, symbolRange: range, targetSource: SourceText) =
        this.FindSymbolHelper(targetDocument, symbolRange, targetSource, true)
    ...
    member this.FindDefinitionOfSymbolAtRange(targetDocument: Document, symbolRange: range, targetSourceText: SourceText) =
        this.FindSymbolHelper(targetDocument, symbolRange, targetSourceText, false)
    
    /// use the targetSymbol to find the first instance of its presence in the provided source file
    member this.FindSymbolDeclarationInFile(targetSymbolUse: FSharpSymbolUse, filePath: string, source: string, options: FSharpProjectOptions, fileVersion:int) = 
        asyncMaybe {
            let! _, checkFileAnswer = checker.ParseAndCheckFileInProject (filePath, fileVersion, source, options, userOpName=this.UserOpName) |> liftAsync
...
module internal GoToDefinitionHelpers =
    ...
    let tryNavigateToTextSpan (document: Document) (textSpan: TextSpan) (statusBar: StatusBar) =
    ...
    let tryNavigateToItem (statusBar: StatusBar) (navigableItem: #INavigableItem option) =
    ...
    let navigateToSymbolDeclarationAsync (targetDocument: Document) (targetSourceText: SourceText) (symbolRange: range) (gtd: GoToDefinition) (statusBar: StatusBar) =
        ...
        |> Async.map (tryNavigateToItem statusBar)

    /// Find the definition location (implementation file/.fs) of the target symbol
    let navigateToSymbolDefinitionAsync (targetDocument: Document) (targetSourceText: SourceText) (symbolRange: range) (gtd: GoToDefinition) (statusBar: StatusBar) =
        gtd.FindDefinitionOfSymbolAtRange(targetDocument, symbolRange, targetSourceText)
        |> Async.map (tryNavigateToItem statusBar)
    ...
    let findDefinitionsTask (checkerProvider: FSharpCheckerProvider) (projectInfoManager: FSharpProjectOptionsManager) (originDocument: Document) (position: int) (gtd: GoToDefinition) (cancellationToken: CancellationToken) =
        asyncMaybe {
            let! parsingOptions, projectOptions = projectInfoManager.TryGetOptionsForEditingDocumentOrProject originDocument
            ...
            let! _, _, checkFileResults = checkerProvider.Checker.ParseAndCheckDocument (originDocument, projectOptions, allowStaleResults=true, sourceText=sourceText, userOpName=gtd.UserOpName)
                
            let! lexerSymbol = Tokenizer.getSymbolAtPosition (originDocument.Id, sourceText, position,originDocument.FilePath, defines, SymbolLookupKind.Greedy, false)
            let idRange = lexerSymbol.Ident.idRange

            let! declarations = checkFileResults.GetDeclarationLocation (fcsTextLineNumber, lexerSymbol.Ident.idRange.EndColumn, textLine.ToString(), lexerSymbol.FullIsland, preferSignature, userOpName=gtd.UserOpName) |> liftAsync
            let! targetSymbolUse = checkFileResults.GetSymbolUseAtLocation (fcsTextLineNumber, idRange.EndColumn, lineText, lexerSymbol.FullIsland, userOpName=gtd.UserOpName)
            ...
            | FSharpFindDeclResult.DeclFound targetRange -> 
                ...
                if lexerSymbol.Range = targetRange then
                    ...
                    if isSignatureFile originDocument.FilePath then
                        ...
                        let! targetRange = gtd.FindSymbolDeclarationInFile(targetSymbolUse, implFilePath, implSourceText.ToString(), projectOptions, implVersion.GetHashCode())
