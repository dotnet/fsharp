...
type internal FSharpMiscellaneousFileService
    ...
    static let mustUpdateProjectReferences (refSourceFiles: string[]) (projectContext: IFSharpWorkspaceProjectContext) =
        ...
        || (refSourceFiles |> Seq.forall projectContext.HasProjectReference |> not)
    ...
    static let mustUpdateMetadataReferences (referencePaths: string[]) (projectContext: IFSharpWorkspaceProjectContext) =
        ...
        || (referencePaths |> Seq.forall projectContext.HasMetadataReference |> not)
    ...
    do
        optionsManager.ScriptUpdated.Add(fun scriptProjectOptions ->
            if scriptProjectOptions.SourceFiles.Length > 0 then
                ...
                let filePath =
                    scriptProjectOptions.SourceFiles.[scriptProjectOptions.SourceFiles.Length - 1]

                let refSourceFiles =
                    scriptProjectOptions.SourceFiles
                    |> Array.take (scriptProjectOptions.SourceFiles.Length - 1)
        ...
        miscFilesWorkspace.DocumentOpened.Add(fun args ->
            ...
            if
                document.Project.IsFSharp
                && workspace.CurrentSolution.GetDocumentIdsWithFilePath(document.FilePath).Length = 0
            then
        ...
        workspace.WorkspaceChanged.Add(fun args ->
            ...
            | WorkspaceChangeKind.ProjectRemoved ->
                ...
                if proj.IsFSharpMiscellaneousOrMetadata then
                    let projRefs = proj.GetAllProjectsThisProjectDependsOn() |> Array.ofSeq
                        ...
                        |> Array.iter (fun proj ->
                            ...
                            | _ ->
                                if proj.IsFSharpMiscellaneousOrMetadata then
                                    ...
                                    | Some doc when not (workspace.IsDocumentOpen(doc.Id)) -> tryRemove doc
                                    | _ -> ())
            | _ -> ())
...
[<Sealed>]
type internal SingleFileWorkspaceMap(miscFileService: FSharpMiscellaneousFileService, rdt: IVsRunningDocumentTable) as this =

    do rdt.AdviseRunningDocTableEvents(this) |> ignore
    ...
    interface IVsRunningDocTableEvents2 with
        ...
        member _.OnAfterAttributeChangeEx(_, grfAttribs, _, _, pszMkDocumentOld, _, _, pszMkDocumentNew) =
            ...
            if
                (grfAttribs &&& (uint32 __VSRDTATTRIB.RDTA_MkDocument)) <> 0u
                && miscFileService.ContainsFile(pszMkDocumentOld)
            then
