...
module internal ClassificationTypes =
    [<Export; Name(FSharpClassificationTypes.PatternCase); BaseDefinition("identifier")>]
    ...
    [<ClassificationType(ClassificationTypeNames = FSharpClassificationTypes.PatternCase)>]
    [<Name(FSharpClassificationTypes.PatternCase)>]
    [<UserVisible(true)>]
    [<Order(After = PredefinedClassificationTypeNames.String)>] 
    type internal FSharpReferenceTypeFormat [<ImportingConstructor>](colorManager: ClassificationColorManager) as self =
        inherit ClassificationFormatDefinition()
        
        do self.DisplayName <- "F# Patterns"
           self.ForegroundColor <- colorManager.GetDefaultColors(FSharpClassificationTypes.ReferenceType)
    ...
    let getClassificationTypeName = function
        ...
        | SemanticClassificationType.Module -> FSharpClassificationTypes.ReferenceType
        | SemanticClassificationType.ValueType -> FSharpClassificationTypes.ValueType
        | SemanticClassificationType.PatternCase -> FSharpClassificationTypes.PatternCase
