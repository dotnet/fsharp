...
module BraceCompletionSessionProviderHelpers =
    ...
    let tryInsertAdditionalBracePair (session: IBraceCompletionSession) openingChar closingChar =
        ...
        && sourceCode.GetText(start, 1).[0] = openingChar
        && sourceCode.GetText(end', 1).[0] = closingChar
...
type BraceCompletionSession
    ...
    member this.Start(cancellationToken: CancellationToken) =
        ...
        if closingPoint = null then
            ...
            let beforeTrackingPoint =
                pos.Snapshot.CreateTrackingPoint(pos.Position, PointTrackingMode.Negative)
                ...
                if openingSnapshotPoint.GetChar() <> openingBrace then
                    ...
                    if not (session.CheckOpeningPoint(this, cancellationToken)) then
                        ...
                        | Some (nextSnapshot) ->
                            ...
                            closingPoint <-
                                subjectBuffer.CurrentSnapshot.CreateTrackingPoint(
                                    closingPoint.GetPoint(nextSnapshot).Position,
                                    PointTrackingMode.Negative
                                )

                            Debug.Assert(
                                closingPoint.GetPoint(nextSnapshot).Position > 0
                                && (SnapshotSpan(closingPoint.GetPoint(nextSnapshot).Subtract(1), 1))
                                    .GetText()
                                    .Equals(closingBrace.ToString()),
    ...
    interface IBraceCompletionSession with
        ...
        member this.PreBackspace handledCommand =
            ...
            if
                ...
                && not this.HasForwardTyping
            then
                use undo = this.CreateUndoTransaction()
                use edit = subjectBuffer.CreateEdit()

                let span =
                    SnapshotSpan(openingPoint.GetPoint(snapshot), closingPoint.GetPoint(snapshot))
        ...
        member _.PreReturn handledCommand = handledCommand <- false
        ...
        member this.PostReturn() =
            ...
            | Some caretPos ->
                ...
                if
                    closingSnapshotPoint.Position > 0
                    && this.HasNoForwardTyping(caretPos, closingSnapshotPoint.Subtract(1))
                then
        ...
        member _.PreDelete handledCommand = handledCommand <- false
...
type ParenthesisCompletionSession() =
    ...
    interface IEditorBraceCompletionSession with
        ...
        member _.CheckOpeningPoint(_session, _cancellationToken) = true
