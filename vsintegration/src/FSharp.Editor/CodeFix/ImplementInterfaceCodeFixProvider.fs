...
type internal FSharpImplementInterfaceCodeFixProvider [<ImportingConstructor>] () =
    ...
    let applyImplementInterface (sourceText: SourceText) state displayContext implementedMemberSignatures entity indentSize verboseMode =
        ...
        | Some index -> sourceText.WithChanges(stubChange, TextChange(TextSpan(index, 0), " }"))
        | None -> sourceText.WithChanges(stubChange)

    let registerSuggestions
        (
    ...
    override _.RegisterCodeFixesAsync context : Task =
        asyncMaybe {
            ...
            let! symbolUse =
                checkFileResults.GetSymbolUseAtLocation(fcsTextLineNumber, symbol.Ident.idRange.EndColumn, lineContents, symbol.FullIsland)
